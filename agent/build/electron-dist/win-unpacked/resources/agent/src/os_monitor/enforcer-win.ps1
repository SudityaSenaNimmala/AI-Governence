        ...
        try
        {
            if (nCode >= 0)
            {
                ...
                if (msg == WM_LBUTTONDOWN || msg == WM_LBUTTONUP)
                {
                    ...
                    // Fallback: when the send button rect is stale/missing but
                    // a block recently fired AND the UIA scan still sees sensitive
                    // content, block the click. The inRect check would miss the
                    // send button when the rect was never cached or got cleared.
                    // _blockUia ensures we don't block after the user edits out
                    // the sensitive data; cooldown ensures we only fire close to
                    // an actual block (not on random clicks minutes later).
                    bool recentBlock = (DateTime.UtcNow.Ticks - _lastBlockFiredTicks) < BLOCK_COOLDOWN;
                    if (_fgIsAi && !inRect && !Disarmed() && PanelEnforceOk() && _blockUia && recentBlock)
                    {
                        if (msg == WM_LBUTTONDOWN) EmitBlock(_app, _uiaPatterns, "click");
