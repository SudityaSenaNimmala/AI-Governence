  ...
  function showCfaiPopup(opts) {
    ...
    if (opts.hardBlock) {
      ...
      if (canRedact) {
        ...
        // Request Access button — shows inline form, submits to server
        const requestBtn = root.querySelector('.cfai-block-request');
        if (requestBtn && opts.requestAccess) {
          requestBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            // Replace button with inline form
            const card = root.querySelector('.cfai-block-card');
            const actionsDiv = root.querySelector('.cfai-block-actions');
            if (actionsDiv) {
              ...
              actionsDiv.querySelector('#cfai-submit-request').addEventListener('click', async (e) => {
                e.stopPropagation();
                const reason = (root.querySelector('#cfai-request-reason')?.value || '').trim();
              ...
              setTimeout(() => root.querySelector('#cfai-request-reason')?.focus(), 0);
            }
          });
        }
      }
