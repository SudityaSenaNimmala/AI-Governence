...
//   • STORE by default. Output size == sum of input sizes. Fine for KBs of text.
//     Pass { compress: true } for archives where size actually matters — the
//     force-installed extension is 45 MB of Tesseract WASM, and every machine in
//     the fleet downloads it. DEFLATE is opt-in rather than default so existing
 ...
 */
export function createZip(files, { compress = false } = {}) {
  const parts = [];
  const centralDir = [];
  let offset = 0;
  ...
  for (const file of files) {
    ...
    // CRC is ALWAYS over the original bytes, never the deflated ones.
    const crc = crc32(data);
    const { time, date } = dosDateTime(file.mtime);

    // Deflate only when it actually wins. Already-compressed payloads (.gz, .png,
    // .wasm) can come out LARGER, and a zip that grew is a worse outcome than one
    // that simply stored.
    let payload = data;
    ...
    central.writeUInt16LE(method, 10);      // must match the local header
