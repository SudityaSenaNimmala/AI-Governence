...
// MCP client/server names and scopes are short identifiers, not literal package
// names — safe to display Title Cased. A few have a non-obvious "official" form
// (GitHub, not Github); everything else gets plain hyphen/underscore→space Title Case.
const KNOWN_SERVICE_LABELS = { github:"GitHub", gitlab:"GitLab", openai:"OpenAI", sqlite:"SQLite", postgresql:"PostgreSQL" };
const SLUG_ACRONYMS = new Set(['mcp','ai','ide','ml','sdk','api','llm','os','ui','sql','id','cli','url','aws','gcp','ssh','ci']);
function slugLabel(value) {
  if (typeof value !== "string" || !value) return value;
  return value.split(/[-_]/).map(w => {
    const lw = w.toLowerCase();
    if (KNOWN_SERVICE_LABELS[lw]) return KNOWN_SERVICE_LABELS[lw];
    return SLUG_ACRONYMS.has(lw) ? lw.toUpperCase() : (w.charAt(0).toUpperCase()+w.slice(1).toLowerCase());
  }).join(' ');
}
...
function SanctionBadge({ status }) { const c={approved:"#22c55e",restricted:"#f59e0b",blocked:"#ef4444",unknown:"#9ca3af"}; return <Badge text={capitalizeWord(status)||"Unknown"} color={c[status]||c.unknown}/>; }
function SeverityBadge({ sev }) { const c={critical:"#ef4444",high:"#f59e0b",medium:"#3b82f6",low:"#22c55e"}; return <Badge text={capitalizeWord(sev)||"—"} color={c[sev]||"#9ca3af"}/>; }
...
function DataTable({ columns, rows, empty, onRow, renderExpanded, isExpanded, paginate }) {
  ...
  return (<div>
    ...
    {(usePaging||paginate>0)&&<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 6px",fontSize:14.7,color:"#4b5563"}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontWeight:500}}>{allRows.length.toLocaleString()} rows</span>
        <select value={pageSize} onChange={e=>{setPageSize(Number(e.target.value));setPage(0);}} style={{padding:"4px 8px",border:"1px solid #e2e5ea",borderRadius:8,fontSize:14.1,background:"#fff",cursor:"pointer",fontFamily:"inherit"}}>
          {[10,25,50,100].map(n=><option key={n} value={n}>{n} / page</option>)}
        </select>
      </div>
      {usePaging&&<div style={{display:"flex",alignItems:"center",gap:4}}>
        <button disabled={page===0} onClick={()=>setPage(page-1)} style={{padding:"5px 12px",border:"1px solid #e2e5ea",borderRadius:8,fontSize:14.1,cursor:page===0?"default":"pointer",opacity:page===0?0.35:1,background:"#fff",fontWeight:600,fontFamily:"inherit",transition:"all 0.15s"}}>Prev</button>
        {pageNums.map((n,i)=>{
          const gap=i>0&&n-pageNums[i-1]>1;
          return <Fragment key={n}>{gap&&<span style={{padding:"0 4px",color:"#9ca3af"}}>...</span>}<button onClick={()=>setPage(n)} style={{padding:"5px 10px",border:"1px solid "+(n===page?"#0052e0":"#e2e5ea"),borderRadius:8,fontSize:14.1,cursor:"pointer",background:n===page?"linear-gradient(135deg, #0052e0, #6366f1)":"#fff",color:n===page?"#fff":"#4b5563",fontWeight:n===page?700:500,fontFamily:"inherit",transition:"all 0.15s",boxShadow:n===page?"0 2px 6px rgba(0,82,224,0.2)":"none"}}>{n+1}</button></Fragment>;
        })}
        <button disabled={page>=totalPages-1} onClick={()=>setPage(page+1)} style={{padding:"5px 12px",border:"1px solid #e2e5ea",borderRadius:8,fontSize:14.1,cursor:page>=totalPages-1?"default":"pointer",opacity:page>=totalPages-1?0.35:1,background:"#fff",fontWeight:600,fontFamily:"inherit",transition:"all 0.15s"}}>Next</button>
      </div>}
    </div>}
  </div>);
}
    ...
    <div className="aihub_drawer_overlay" onClick={onClose} role="dialog" aria-modal="true">
      <aside className="aihub_drawer" onClick={e=>e.stopPropagation()}>
        ...
        <div className="aihub_drawer_body">
          ...
          {state.status==="error" && <div style={{padding:16}}><div className="aihub_error"><AlertTriangle size={14}/> {state.error}</div><p className="aihub_text_muted" style={{fontSize:14.7,marginTop:10}}>Older events captured before content storage was enabled won't have a preview available.</p></div>}
          ...
          {state.status==="ok" && state.kind==="binary" && (
            <div style={{padding:28,textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
              ...
              <div className="aihub_text_muted" style={{marginTop:4,fontSize:14.7}}>{state.contentType||"application/octet-stream"} · can't render inline</div>
    ...
    <div style={{padding:"16px 20px"}}>
      {Array.isArray(matches)&&matches.length>0 && (
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12,alignItems:"center"}}>
          <span className="aihub_text_muted" style={{fontSize:14.1,fontWeight:600}}>Matched:</span>
      ...
      <div className="aihub_text_muted" style={{marginTop:10,fontSize:14.1}}>{text.length.toLocaleString()} chars · {contentType}</div>
    </div>
  );
}
...
function Donut({ segments, label, size=190, thickness=38 }) {
  ...
  return (<div style={{position:"relative",display:"inline-block",flexShrink:0}}>
    ...
    {tip&&<div style={{
      ...
      fontSize:14.7,fontWeight:600,whiteSpace:"nowrap",pointerEvents:"none",
      ...
      <div style={{display:"flex",justifyContent:"space-between",gap:16,marginTop:4,fontSize:14.1,color:"rgba(255,255,255,0.65)"}}>
      ...
      {views.map((vw,i)=>(
        <button key={vw.key} onClick={e=>{e.stopPropagation();setActive(i);}}
          style={{padding:"4px 12px",borderRadius:20,border:"1px solid "+(i===active?"#0052e0":"#e2e5ea"),
            ...
            fontSize:14.1,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>
          {vw.label}
        </button>
      ))}
    </div>
      ...
      <div className="aihub_card">
        ...
        {attn.length===0
          ? <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"24px 0",color:"#9ca3af"}}>
              ...
              <p style={{margin:"10px 0 0",fontSize:15.2,fontWeight:500,color:"#4b5563"}}>{warn.length>0?"Some panels could not load.":"All clear — nothing outstanding."}</p>
            </div>
...
// What KIND of egress a captured event is, read from `metadata.surface_kind`.
//
// The desktop agent's Outlook and OneDrive/SharePoint guardrails stamp
// "data_egress": those events are a mail send or a file sync, not a prompt to a
// model. They belong on a DLP screen — that is the whole point of capturing them
// — but folding them into the AI counters would inflate every AI-usage number
// here with traffic that never touched an AI service, so they are tagged per row
// and can be scoped in or out.
//
// ABSENT MEANS AI SERVICE. Every browser-extension event, and every row written
// before the desktop guardrails existed, carries no `surface_kind` at all;
// defaulting those to data egress would empty the AI view and silently reclassify
// years of history. Same reasoning as SURFACE_META further down this file.
const EVENT_SURFACE_KINDS = {
  ai_service:  { label:"AI Service",  color:"#0052e0", hint:"Prompt or upload sent to an AI service." },
  data_egress: { label:"Data Egress", color:"#0891b2", hint:"Business data leaving through a non-AI channel — Outlook mail, or OneDrive/SharePoint sync — captured by the desktop agent." },
};
function eventSurfaceKind(ev) {
  // Top-level field checked as well as metadata: the /dlp routes spread the whole
  // stored row before attaching parsed metadata, so either shape can arrive.
  const k = ev?.metadata?.surface_kind || ev?.surface_kind;
  return k && EVENT_SURFACE_KINDS[k] ? k : "ai_service";
}
// Render helper, not a component: a `{ row }` signature trips react/prop-types on
// every field it reads (see surfaceBadge below for the same reasoning).
function surfaceKindBadge(row) {
  const meta = EVENT_SURFACE_KINDS[eventSurfaceKind(row)];
...
function DLPView() {
  ...
  // Surface-kind scope, applied BEFORE the severity narrowing, the grouping and
  // every counter below — so each card, group count and table on this screen
  ...
  return (<div>
    ...
    {/* Event Kind filter (surface_kind-based split between AI-service traffic and
        ...
        the server stamps `surface_kind` on events yet, so every row would default
        to "ai_service" (see eventSurfaceKind()) and this UI would show a permanently
        empty "Data Egress Only" filter and a counter stuck at "0 of N". Re-enable
        once the desktop agent's Outlook/OneDrive guardrail actually stamps the
        field — the EVENT_SURFACE_KINDS/eventSurfaceKind/surfaceKindBadge plumbing
        below is already in place for that. */}
  ...
  return (<>
    ...
    {state.kind==="binary" && (
      <div style={{padding:16,textAlign:"center"}}>
        ...
        <div style={{marginTop:6,fontWeight:600,color:"#374151",fontSize:15.2}}>{filename||"Binary file"}</div>
        <div className="aihub_text_muted" style={{marginTop:2,fontSize:14.1}}>{state.contentType||"application/octet-stream"} · cannot render inline</div>
        <a href={url} download={filename||"download.bin"} className="aihub_dl_btn">Download file</a>
      </div>
    )}
  </>);
  ...
  const transcriptCard=(
    <div className="aihub_card">
      ...
      {!err && messages.length>0 && (<>
        ...
        {hasPlayer && (
          <p className="aihub_text_muted" style={{fontSize:14.1,margin:"0 0 12px"}}>
    ...
    <div className="aihub_filter_bar">
      ...
      <div className="aihub_filter_group">
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {!sevAvailable && <span className="aihub_text_muted" style={{fontSize:14.1}}>per-session severity not reported yet</span>}
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h2 style={{ margin: 0, fontSize:21.9 }}>Copilot Readiness Assessment</h2>
          <p className="aihub_text_muted" style={{ margin: "4px 0 0", fontSize:15.2 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          {oauthKeys.length > 1 && (
            <select value={selectedKey} onChange={e => setSelectedKey(e.target.value)}
              style={{ padding: "8px 12px", border: "1px solid #d1d5db", borderRadius: 8, fontSize:15.2 }}>
          ...
          <button onClick={runScan} disabled={scanning || !selectedKey}
            style={{ background: scanning ? "#9ca3af" : "#2563eb", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontSize:15.8, fontWeight: 600, cursor: scanning ? "default" : "pointer", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {oauthKeys.length === 0 && scan && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, padding: "9px 14px",
                      borderRadius: 8, background: "#fffbeb", border: "1px solid #fde68a", fontSize:14.7, color: "#92400e" }}>
      ...
      {scan && scan.status === "completed" && (<>
        ...
        <div style={{ background: sevColor[scan.riskLevel] + "10", border: "1px solid " + sevColor[scan.riskLevel] + "40", borderRadius: 12, padding: 20, marginBottom: 20, display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ width: 60, height: 60, borderRadius: "50%", background: sevColor[scan.riskLevel] + "20", color: sevColor[scan.riskLevel], display: "flex", alignItems: "center", justifyContent: "center", fontSize:31.4, fontWeight: 700 }}>
            {scan.riskLevel === "critical" ? "!" : scan.riskLevel === "high" ? "⚠" : scan.riskLevel === "none" ? "✓" : "~"}
          </div>
          <div>
            <div style={{ fontSize:24.7, fontWeight: 700, color: sevColor[scan.riskLevel], textTransform: "uppercase" }}>{scan.riskLevel} Risk</div>
            <div style={{ fontSize:15.2, color: "#475569", marginTop: 2 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          {[["critical", scan.summary.critical], ["high", scan.summary.high], ["medium", scan.summary.medium], ["low", scan.summary.low]].map(([sev, count]) => (
            <div key={sev} style={{ padding: "6px 12px", borderRadius: 6, background: sevColor[sev] + "14", color: sevColor[sev], fontSize:14.7, fontWeight: 600 }}>
              {sev}: {count}
            </div>
          ))}
        </div>
        ...
        <DataTable
          columns={[
            ...
            { label: "Finding", render: r => <div><div className="aihub_text_primary" style={{ fontSize:15.2 }}>{r.title}</div><div className="aihub_text_muted" style={{ fontSize:14.1, marginTop: 2 }}>{r.description.length > 120 ? r.description.slice(0, 120) + "…" : r.description}</div></div> },
            { label: "Exposed To", render: r => <span style={{ fontSize:14.7 }}>{r.exposedTo}</span> },
            { label: "Remediation", render: r => <span className="aihub_text_muted" style={{ fontSize:14.1 }}>{r.remediation.length > 80 ? r.remediation.slice(0, 80) + "…" : r.remediation}</span> },
          ]}
          rows={scan.findings || []}
        ...
        {/* Scan metadata */}
        <div className="aihub_text_muted" style={{ fontSize:14.1, marginTop: 16 }}>
      ...
      {scan && scan.status === "running" && (
        <div style={{ textAlign: "center", padding: 60 }}>
          ...
          <div style={{ fontSize:18, fontWeight: 600, marginBottom: 4 }}>Scanning your Microsoft 365 environment...</div>
      ...
      {!scan && oauthKeys.length > 0 && !scanning && (
        <div style={{ textAlign: "center", padding: 60, background: "#f8fafc", borderRadius: 12, border: "1px solid #e5e7eb" }}>
          ...
          <div style={{ fontSize:18, fontWeight: 600, marginBottom: 4 }}>No assessment run yet</div>
          ...
          <div style={{ fontSize:14.7, color: "#6b7280", maxWidth: 500, margin: "0 auto", lineHeight: 1.6 }}>
...
function MultiSelect({options,value=[],onChange,label}) {
  return (<div style={{marginBottom:10}}>
    {label&&<div style={{fontSize:14.7,fontWeight:600,color:"#374151",marginBottom:4}}>{label}</div>}
    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
      {options.map(o=>{
        const sel=value.includes(o);
        return <button key={o} type="button" onClick={()=>onChange(sel?value.filter(v=>v!==o):[...value,o])}
          style={{padding:"3px 10px",borderRadius:6,fontSize:14.1,fontWeight:600,border:"1px solid",cursor:"pointer",
  ...
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
    <div style={{background:"#fff",borderRadius:14,padding:24,width:560,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
        <h3 style={{margin:0,fontSize:18,fontWeight:700}}>{isEdit?"Edit Rule":"Create Routing Rule"}</h3>
        <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer"}}><X size={18} color="#6b7280"/></button>
      </div>

      <div style={{marginBottom:12}}>
        <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Rule Name *</label>
      ...
      <div style={{display:"flex",gap:12,marginBottom:12}}>
        <div style={{flex:1}}>
          <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Priority (lower = first)</label>
          <input type="number" value={priority} onChange={e=>setPriority(e.target.value)} style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,marginTop:4,boxSizing:"border-box"}}/>
        </div>
        <div style={{flex:1,display:"flex",alignItems:"flex-end",paddingBottom:4}}>
          <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:15.2}}>
      ...
      <div style={{background:"#f5f6f8",borderRadius:10,padding:14,marginBottom:14}}>
        <div style={{fontSize:15.2,fontWeight:700,color:"#111827",marginBottom:10}}>Conditions <span style={{fontWeight:400,color:"#9ca3af"}}>(all must match)</span></div>
        ...
        <div style={{marginBottom:10}}>
          <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Original Model (comma-separated patterns)</label>
          <input value={condModel} onChange={e=>setCondModel(e.target.value)} placeholder="e.g. gpt-4, claude-opus"
            style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,marginTop:4,boxSizing:"border-box"}}/>
        </div>
        <div style={{display:"flex",gap:12}}>
          <div style={{flex:1}}>
            <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Tokens &gt;</label>
      ...
      <div style={{background:"#f0f9ff",borderRadius:10,padding:14,marginBottom:18}}>
        ...
        <div>
          <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Target Host <span style={{fontWeight:400,color:"#9ca3af"}}>(optional, for private endpoints)</span></label>
  ...
  return (<div>
    ...
    <div style={{display:"flex",gap:2,marginBottom:16,borderBottom:"2px solid #eff1f3"}}>
      {tabs.map(t=><button key={t.id} onClick={()=>setTab(t.id)}
        style={{padding:"8px 18px",fontSize:15.2,fontWeight:tab===t.id?700:500,border:"none",borderBottom:tab===t.id?"2px solid #0044cc":"2px solid transparent",
    ...
    {tab==="overview"&&(<div>
      {analytics?.total_routed===0 ? (
        <div className="aihub_card" style={{textAlign:"center",padding:"40px 20px"}}>
          ...
          <button onClick={()=>{setTab("rules");setShowRuleForm(true);}} style={{padding:"8px 20px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:15.2,fontWeight:600}}>
        ...
        <div className="aihub_two_col">
          ...
          <div>
            <div className="aihub_card" style={{marginBottom:16}}>
              <h4 style={{margin:"0 0 12px",fontSize:15.8,fontWeight:700}}>By Rule</h4>
              <BarChart data={(analytics?.by_rule||[]).map(r=>({label:r.name||"Unknown",count:r.count}))} lk="label" vk="count"/>
            </div>
            <div className="aihub_card">
              <h4 style={{margin:"0 0 12px",fontSize:15.8,fontWeight:700}}>By Complexity</h4>
              <div style={{display:"flex",gap:8}}>
                {(analytics?.by_complexity||[]).map(c=>
                  <div key={c.complexity} style={{flex:1,textAlign:"center",padding:12,borderRadius:8,background:"#f5f6f8"}}>
                    <div style={{fontSize:21.9,fontWeight:700,color:"#111827"}}>{c.count}</div>
                    <div style={{fontSize:14.1,color:"#6b7280",textTransform:"capitalize"}}>{c.complexity||"unknown"}</div>
                  </div>
                )}
              </div>
            </div>
    ...
    {tab==="rules"&&(<div>
      <div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}>
        <button onClick={()=>{setEditRule(null);setShowRuleForm(true);}}
          style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:15.2,fontWeight:600}}>
    ...
    {tab==="endpoints"&&(<div>
      ...
      {showEpForm&&(
        <div className="aihub_card" style={{marginBottom:16,background:"#f5f6f8"}}>
          <h4 style={{margin:"0 0 12px",fontSize:15.8,fontWeight:700}}>Register Endpoint</h4>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <div>
              <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Name *</label>
              <input value={epName} onChange={e=>setEpName(e.target.value)} placeholder="e.g. Azure OpenAI Frankfurt"
                style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,marginTop:4,boxSizing:"border-box"}}/>
            </div>
            <div>
              <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Provider</label>
            ...
            <div>
              <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Host</label>
              <input value={epHost} onChange={e=>setEpHost(e.target.value)} placeholder="e.g. my-company.openai.azure.com"
                style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,marginTop:4,boxSizing:"border-box"}}/>
            </div>
            <div>
              <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Models (comma-separated)</label>
              <input value={epModels} onChange={e=>setEpModels(e.target.value)} placeholder="e.g. gpt-4o, gpt-4o-mini"
                style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,marginTop:4,boxSizing:"border-box"}}/>
            </div>
            <div>
              <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Region</label>
            ...
            <div style={{display:"flex",alignItems:"flex-end",gap:8}}>
              <button onClick={saveEndpoint} disabled={!epName.trim()}
                style={{padding:"8px 20px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:15.2,fontWeight:600,opacity:!epName.trim()?0.5:1}}>Save</button>
              <button onClick={()=>setShowEpForm(false)} style={{padding:"8px 20px",borderRadius:8,border:"1px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:15.2}}>Cancel</button>
            </div>
          </div>
        </div>
      )}
  ...
  if (score == null || level === "not_assessed" || !level) {
    return <span title="No risk assessment has run for this subject yet"
                 style={{display:"inline-flex",alignItems:"center",gap:6,padding:"3px 10px",borderRadius:8,
                         fontSize:14.7,fontWeight:600,background:"#f3f4f6",color:"#6b7280",border:"1px dashed #d1d5db"}}>
      Not assessed
    </span>;
  }
  const c = RISK_COLORS[level] || "#6b7280";
  return <span style={{display:"inline-flex",alignItems:"center",gap:6,padding:"3px 10px",borderRadius:8,fontSize:14.7,fontWeight:700,background:c+"14",color:c,border:"1px solid "+c+"30"}}>
...
function FactorRow({ label, factor }) {
  ...
  return <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
    <div style={{width:140,fontSize:14.7,color:"#6b7280",flexShrink:0}}>{label}</div>
    <div style={{flex:1,height:6,background:"#f3f4f6",borderRadius:3,overflow:"hidden"}}>
      <div style={{width:pct+"%",height:"100%",background:c,borderRadius:3}}/>
    </div>
    <div style={{width:50,fontSize:14.7,fontWeight:600,color:c,textAlign:"right"}}>{factor.raw}</div>
    <div style={{width:40,fontSize:14.1,color:"#9ca3af",textAlign:"right"}}>{factor.weight}%</div>
  </div>;
}
...
function RiskRowDetail({ detail, loading }) {
  ...
  const H=({children})=><div style={{fontSize:14.1,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:".03em",marginBottom:8}}>{children}</div>;
  ...
  return (<div style={{padding:"16px 20px",borderTop:"1px solid #e5e7eb"}}>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:22}}>
      ...
      <div>
        ...
        <div className="aihub_text_muted" style={{fontSize:14.1,marginTop:8}}>
      ...
      <div>
        {detail.history?.length>0 && <div style={{marginBottom:18}}>
          ...
          <div className="aihub_text_muted" style={{fontSize:14.1,marginTop:4}}>
        ...
        {detail.recent_events?.length>0 && <div>
          ...
          <div style={{maxHeight:180,overflowY:"auto"}}>
            {detail.recent_events.slice(0,8).map((ev,i)=>(
              <div key={i} style={{padding:"6px 0",borderBottom:"1px solid #f3f4f6",fontSize:14.7}}>
        ...
        {!detail.history?.length && !detail.recent_events?.length &&
          <div className="aihub_text_muted" style={{fontSize:14.7}}>No score history or recent events recorded for this person yet.</div>}
      </div>
...
// ── Guardrail policy vocabularies (ai_platforms.surface / .capture_mode) ─────
//
// The two fields that decide WHICH enforcer owns a host and HOW HARD it enforces.
// Both lists mirror the server's allow-lists in server/src/routes/ai-platforms.js
// (VALID_SURFACE / VALID_CAPTURE_MODE); a value outside them is rejected with a
// 400, so the two must stay in sync.
//
// Until the row control below existed, nothing in the admin UI ever wrote
// `surface`: every row sat on the server's "browser" default, which meant the
// desktop agent's Outlook and OneDrive/SharePoint guardrails could not be turned
// on for a host from anywhere in the product. That is the gap these close.
const PLATFORM_SURFACES = [
  { value:"browser", label:"Browser",       hint:"Enforced by the browser extension only." },
  { value:"desktop", label:"Desktop",       hint:"Enforced by the desktop agent — required for the Outlook and OneDrive/SharePoint guardrails." },
  { value:"cli",     label:"CLI",           hint:"Enforced by the CLI wrapper only." },
  { value:"all",     label:"All Surfaces",  hint:"Enforced everywhere — browser extension, desktop agent and CLI wrapper." },
];
const CAPTURE_MODES = [
  { value:"observe",        label:"Observe",        hint:"Record the activity and block nothing." },
  { value:"block_critical", label:"Block Critical", hint:"Refuse the send when a critical finding is detected; everything else is recorded." },
  { value:"hold",           label:"Hold",           hint:"Queue the send for admin approval instead of letting it through." },
];
function surfaceOptionHint(v) { return (PLATFORM_SURFACES.find(s=>s.value===v)||{}).hint||""; }
function surfaceLabel(v) { return (PLATFORM_SURFACES.find(s=>s.value===v)||{}).label||slugLabel(v); }
function captureModeHint(v)   { return (CAPTURE_MODES.find(m=>m.value===v)||{}).hint||""; }
// Shared heading style for the blocks inside an expanded inventory row. A plain
// object rather than a component so the closures inside AIRegistryView can use
// the same heading as RegistryRowDetail without a second `({children})`
// signature (which react/prop-types flags on this file's config).
const DETAIL_H_STYLE = {fontSize:14.1,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:".03em",marginBottom:6};
...
// Category values are hyphenated slugs ("mcp-server", "ide-assistant") — display
// them word-cased with known abbreviations fully capitalized (MCP, IDE, ML) rather
// than printing the raw slug verbatim.
const CATEGORY_ACRONYMS = new Set(['mcp', 'ide', 'ml', 'ai', 'sdk', 'api', 'llm']);
function categoryLabel(cat) {
  if (!cat) return cat;
  return cat.split('-').map(w => CATEGORY_ACRONYMS.has(w) ? w.toUpperCase() : w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
}
...
function RegistryStatusBadge({ status }) {
  ...
  return <span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"2px 10px",borderRadius:8,fontSize:14.1,fontWeight:700,background:c+"14",color:c,border:"1px solid "+c+"30"}}>{label}</span>;
}
...
function AIRegistryView() {
  ...
  // already fetches them to cross-reference statuses; they are kept in state
  // because `surface` and `capture_mode` are fields on the PLATFORM row, not on
  // the registry row, so the expansion has to read and write them from here.
  // Seeded to [] rather than null: unlike the monitoring lists, a missing
  // platform row makes the control absent, never wrongly OFF, so there is no
  // unknown state to distinguish.
  const [platforms,setPlatforms]=useState([]);
  const [policyPendingHosts,setPolicyPendingHosts]=useState(()=>new Set());
  const [policyErrs,setPolicyErrs]=useState({});   // host → message
  ...
  // ── Guardrail policy for one host ─────────────────────────────────────────
  const platformByHost=useMemo(()=>new Map(platforms.map(p=>[p.host,p])),[platforms]);

  // The ai_platforms row an inventory row's policy belongs to, or null.
  //
  // Deliberately conservative. Catalog rows carry their own `host`, so those are
  // exact. A registry row may match several platform hosts at once (a Copilot
  // Studio agent's matched_hosts spans the whole Microsoft suite), and picking
  // one of them would silently rewrite the wrong host's enforcement policy —
  // a governance control must not guess which row it is editing. Ambiguous rows
  // get no control, which reads as "set this on the platform itself".
  const platformFor=(row)=>{
    if(!row) return null;
    if(row.host) return platformByHost.get(row.host)||null;
    if(row.platform&&platformByHost.has(row.platform)) return platformByHost.get(row.platform);
    const hits=(row.matched_hosts||[]).filter(h=>platformByHost.has(h));
    return hits.length===1?platformByHost.get(hits[0]):null;
  };

  // PATCH one policy field on a platform row. Not folded into setRowStatus:
  // that one also targets /registry/:id/status for non-catalog rows and mirrors
  // decisions into the agent blocklist, whereas this only ever writes the two
  // enforcement fields on ai_platforms and never touches governed/blocked.
  const setPlatformPolicy=async(plat,patch)=>{
    ...
    try{
      ...
      // The server validates both fields against its own allow-lists and
      // normalizes a null to its default, so its answer is the state of record —
      // and a select left showing a value the server rejected would be a lie
      // about what the endpoints are enforcing.
      setPlatforms(prev=>prev.map(p=>p.host===host?{...p,...out}:p));
    } catch(e){
      setPolicyErrs(prev=>({...prev,[host]:e?.message||"Update failed"}));
    } finally {
      setPolicyPendingHosts(prev=>{ const next=new Set(prev); next.delete(host); return next; });
    }
  };

  // The guardrail block for one row. A closure, like renderDlpMonitor, so it
  // reads exactly the platform list the rest of this view was built from.
  const renderPlatformPolicy=(row)=>{
    const plat=platformFor(row);
    if(!plat) return null;
    const pending=policyPendingHosts.has(plat.host);
    const rowErr=policyErrs[plat.host];
    // Missing means browser on the server side too (POST and PATCH both default
    // it), so the select shows browser rather than an empty option that would
    // only ever mean "row predates this control".
    ...
    return (<div style={{marginBottom:14}}>
      ...
      <div className="aihub_text_muted" style={{fontSize:13.6,marginTop:6,maxWidth:660}}>
        ...
        {" "}Endpoints pick the change up on their next sync.
        {/* Stated because it is the one non-obvious consequence: the extension
            polls /ai-platforms?surface=browser, which the server answers with
            browser + all only. A desktop- or CLI-only host drops off that list. */}
        {!browserServed&&<> The browser extension only receives hosts on the
          <Mono> browser</Mono> or <Mono>all</Mono> surfaces, so it will stop enforcing this one.</>}
        {/* The desktop agent arms an egress surface off `governed` and only reads
            capture_mode from governed rows, so on an ungoverned row the mode is
            stored and ignored. Said here rather than letting an admin set "hold"
            and believe a mail send is being held. */}
        {!plat.governed&&<> This row is not governed, so the capture mode is saved
          but not applied — endpoints only read it from governed rows.</>}
      </div>
  ...
  const renderDlpMonitor=(row)=>{
    ...
    // Carries its own heading (rather than being wrapped in one by
    // RegistryRowDetail) because the expansion now holds two optional blocks and
    // either can be absent — a caller-supplied heading would leave an empty
    // "Sensitive-data monitoring" label on every host row that has no agent.
    return (<div style={{marginBottom:14}}>
      <div style={DETAIL_H_STYLE}>Sensitive-data monitoring</div>
      <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        <button type="button" role="switch" aria-checked={on} disabled={disabled} title={why}
          ...
          style={{display:"inline-flex",alignItems:"center",gap:8,padding:"6px 12px",borderRadius:8,
                  fontSize:14.7,fontWeight:600,fontFamily:"inherit",
  ...
  return (<div>
    ...
    <div className="aihub_card" style={{display:"flex",flexWrap:"wrap",gap:10,alignItems:"center",padding:12,marginBottom:16}}>
      <div className="aihub_search_box" style={{flex:1,minWidth:200}}>
        <Search size={14}/><input placeholder="Search name, vendor, owner..." value={search} onChange={e=>setSearch(e.target.value)} style={{border:"none",outline:"none",flex:1,fontSize:15.2}}/>
      </div>
      <select value={filterType} onChange={e=>setFilterType(e.target.value)} style={{padding:"6px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:14.7,fontWeight:600}}>
        <option value="">All</option>
        <option value="tool">AI Tools</option>
        <option value="agent">AI Agents</option>
      </select>
      <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{padding:"6px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:14.7}}>
        <option value="">All Statuses</option>
        <option value="approved">Allowed</option>
        <option value="unknown">Unreviewed</option>
        <option value="blocked">Blocked</option>
      </select>
      <select value={filterCategory} onChange={e=>setFilterCategory(e.target.value)} style={{padding:"6px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:14.7}}>
        <option value="">All Categories</option>
        {categories.map(c=><option key={c} value={c}>{categoryLabel(c)}</option>)}
      </select>
      <select value={filterRisk} onChange={e=>setFilterRisk(e.target.value)} style={{padding:"6px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:14.7}}>
 ...
 *
 * `surface` IS collected, unlike category and governance_note, because it decides
 * which endpoint ever sees the row: the extension polls `?surface=browser`, which
 * the server answers with browser + all only. A desktop-only host — Outlook,
 * OneDrive/SharePoint — added on the browser default would sit in the catalog
 * being enforced by nothing. It still defaults to browser, so the common case is
 * one fewer decision; capture_mode stays off this form for the reason above.
 */
function AddPlatformForm({ onDone }) {
  ...
  const submit=async()=>{
    ...
    try {
      ...
      setOk(`${normalized} added on the ${surface} surface — endpoints will pick it up on their next sync.`);
      setHost(""); setProduct(""); setVendor(""); setSurface("browser");
      onDone?.();
    } catch(e){ setErr(e.message); }
  ...
  return (<div className="aihub_card" style={{marginBottom:16,borderLeft:"3px solid #0044cc"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12,marginBottom:12}}>
      <div>
        ...
        {host && !valid && <div style={{fontSize:14.1,color:"#b91c1c",marginTop:4}}>Not a valid domain.</div>}
        {host && valid && normalized!==host.trim().toLowerCase() &&
          <div style={{fontSize:14.1,color:"#6b7280",marginTop:4}}>Will be saved as <Mono>{normalized}</Mono></div>}
      </div>
      <div>
      ...
      <div>
        <label style={lbl}>Enforcement surface</label>
        <select style={{...field,background:"#fff",cursor:"pointer"}} aria-label="Enforcement surface"
                value={surface} onChange={e=>setSurface(e.target.value)}>
          {PLATFORM_SURFACES.map(s=><option key={s.value} value={s.value}>{s.label}</option>)}
        </select>
        <div style={{fontSize:13.6,color:"#6b7280",marginTop:4}}>{surfaceOptionHint(surface)}</div>
      </div>
    </div>
    ...
    <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
      ...
      <span className="aihub_text_muted" style={{fontSize:14.1}}>
        Starts in <strong>observe</strong> mode — usage is recorded, nothing is blocked.
        Block it, or change its surface and capture mode, afterwards from its row.
        Endpoints sync within a few minutes.
      </span>
    </div>
  </div>);
}
...
function RegistryRowDetail({ row, onStatus, pending, children }) {
  ...
  const H=({children})=><div style={DETAIL_H_STYLE}>{children}</div>;
  return (<div style={{padding:"16px 20px",borderTop:"1px solid #e5e7eb"}}>
    {row.description&&<div style={{fontSize:14.7,color:"#374151",marginBottom:12}}>{row.description}</div>}
    ...
    {/* The other governance decisions for this row, supplied as children by the
        caller rather than built here so they read exactly the same live state as
    ...
    {row.matched_hosts?.length>0&&<div style={{marginBottom:14}}>
      <H>Enforced hosts</H>
      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
        {row.matched_hosts.map((h,i)=><span key={i} style={{fontSize:14.1,fontFamily:"monospace",padding:"2px 8px",borderRadius:5,background:row.status==='blocked'?"#fef2f2":"#f0fdf4",border:"1px solid "+(row.status==='blocked'?"#fca5a5":"#bbf7d0"),color:row.status==='blocked'?"#dc2626":"#16a34a"}}>{h}</span>)}
      </div>
      <div className="aihub_text_muted" style={{fontSize:13.6,marginTop:4}}>Blocking/allowing this system affects {row.matched_hosts.length===1?"this host":"all "+row.matched_hosts.length+" hosts"} in the extension.</div>
    </div>}
    ...
    <div style={{marginBottom:14}}>
      ...
      {row.risk_score!=null?(<>
        ...
        {row.risk_factors?.length>0
          ? <ul style={{margin:"10px 0 0",paddingLeft:18,fontSize:14.1,color:"#4b5563"}}>
          ...
          : <div className="aihub_text_muted" style={{fontSize:14.1,marginTop:8}}>Scored, but no individual signals were recorded for this system.</div>}
        {row.risk_basis==="platform_baseline"&&<div style={{fontSize:14.1,color:"#b45309",marginTop:8}}>
    ...
    {row.data_access?.length>0&&<div style={{marginBottom:14}}>
      <H>Data access</H>
      <div style={{display:"flex",flexWrap:"wrap",gap:4}}>{row.data_access.map((d,i)=><Tag key={i} text={slugLabel(d)} color="#ef4444"/>)}</div>
    </div>}

    {row.permissions?.length>0&&<div style={{marginBottom:14}}>
      <H>Permissions</H>
      <div style={{maxHeight:110,overflowY:"auto",fontSize:14.1,color:"#6b7280"}}>
    ...
    <div>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        <div style={{background:"#fff",border:"1px solid #e5e7eb",padding:"8px 16px",borderRadius:8,textAlign:"center",minWidth:110}}>
          <div style={{fontSize:18.6,fontWeight:700}}>{(row.activity?.total||0).toLocaleString()}</div>
          <div className="aihub_text_muted">Total events</div>
        </div>
        <div style={{background:"#fff",border:"1px solid #e5e7eb",padding:"8px 16px",borderRadius:8,textAlign:"center",minWidth:110}}>
          <div style={{fontSize:18.6,fontWeight:700}}>{row.activity?.unique_users||0}</div>
...
function hostAppLine(row) {
  ...
  return <div className="aihub_text_muted" style={{fontSize:14.1}}>in {row.tool_name || row.tool_host}</div>;
}
...
function AccessRequestsView() {
  ...
  return (<div>
    ...
    <div style={{display:"flex",gap:2,marginBottom:16,borderBottom:"2px solid #eff1f3"}}>
      {tabs.map(t=><button key={t.id} onClick={()=>setTab(t.id)}
        style={{padding:"10px 20px",fontSize:15.2,fontWeight:tab===t.id?700:500,border:"none",borderBottom:tab===t.id?"2px solid #0052e0":"2px solid transparent",
    ...
    {tab==="pending"&&(<div>
      {pending.length===0?(
        <div className="aihub_card" style={{textAlign:"center",padding:"40px 20px"}}>
          ...
          <p style={{color:"#9ca3af",fontSize:15.2}}>When employees request access to a blocked tool, their requests appear here.</p>
        </div>
      ):(
        <div className="aihub_card">
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                  <span style={{fontSize:18,fontWeight:700}}>{primaryToolLabel(r)}</span>
            ...
            {approving===r.id?(
              <div style={{background:"#f0f9ff",borderRadius:10,padding:14,marginTop:8}}>
                {/* Scope-aware, and ONLY for agent-scoped requests: an app-scoped
                    ...
                    stops at this agent. */}
                {agentLabelOf(r)&&(
                  <div style={{fontSize:14.7,color:"#374151",lineHeight:1.6,marginBottom:10}}>
                ...
                <div style={{fontSize:15.2,fontWeight:600,marginBottom:8}}>Set expiry (required)</div>
                <div style={{display:"flex",gap:8,marginBottom:10}}>
                  <button onClick={()=>setExpiryMode("hours")} style={{padding:"5px 12px",borderRadius:6,fontSize:14.7,border:"1px solid",cursor:"pointer",background:expiryMode==="hours"?"#0044cc14":"#fff",color:expiryMode==="hours"?"#0052e0":"#6b7280",borderColor:expiryMode==="hours"?"#0044cc40":"#e2e5ea"}}>Hours</button>
                  <button onClick={()=>setExpiryMode("date")} style={{padding:"5px 12px",borderRadius:6,fontSize:14.7,border:"1px solid",cursor:"pointer",background:expiryMode==="date"?"#0044cc14":"#fff",color:expiryMode==="date"?"#0052e0":"#6b7280",borderColor:expiryMode==="date"?"#0044cc40":"#e2e5ea"}}>Date</button>
                </div>
                {expiryMode==="hours"?(
                  <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:10}}>
                    {["4","8","24","72","168"].map(h=>
                      <button key={h} onClick={()=>setExpiryHours(h)} style={{padding:"4px 10px",borderRadius:6,fontSize:14.1,border:"1px solid",cursor:"pointer",background:expiryHours===h?"#0044cc14":"#fff",color:expiryHours===h?"#0052e0":"#6b7280",borderColor:expiryHours===h?"#0044cc40":"#e2e5ea"}}>{h==="168"?"7 days":h+"h"}</button>
                    )}
                    <input type="number" value={expiryHours} onChange={e=>setExpiryHours(e.target.value)} style={{width:60,padding:"4px 8px",border:"1px solid #e5e7eb",borderRadius:6,fontSize:14.7}} min="1"/>
                    <span style={{fontSize:14.7,color:"#6b7280"}}>hours</span>
                  </div>
                ):(
                  <input type="datetime-local" value={expiryDate} onChange={e=>setExpiryDate(e.target.value)} style={{padding:"6px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:14.7,marginBottom:10}}/>
                )}
                <input value={reviewNote} onChange={e=>setReviewNote(e.target.value)} placeholder="Note (optional)" style={{width:"100%",padding:"6px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:14.7,marginBottom:10,boxSizing:"border-box"}}/>
                <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                  <button onClick={()=>{setApproving(null);setReviewNote("");}} style={{padding:"6px 16px",borderRadius:8,border:"1px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:14.7}}>Cancel</button>
                  <button onClick={()=>reject(r.id)} style={{padding:"6px 16px",borderRadius:8,border:"1px solid #ef444440",background:"#ef444414",color:"#ef4444",cursor:"pointer",fontSize:14.7,fontWeight:600}}>Reject</button>
                  <button onClick={()=>approve(r.id)} style={{padding:"6px 16px",borderRadius:8,border:"none",background:"#22c55e",color:"#fff",cursor:"pointer",fontSize:14.7,fontWeight:600}}>Approve</button>
                </div>
              </div>
            ):(
              <div style={{display:"flex",gap:8,marginTop:8}}>
                <button onClick={()=>setApproving(r.id)} style={{padding:"6px 16px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:14.7,fontWeight:600}}>Review</button>
                <button onClick={()=>reject(r.id)} style={{padding:"6px 16px",borderRadius:8,border:"1px solid #ef444440",background:"#ef444414",color:"#ef4444",cursor:"pointer",fontSize:14.7,fontWeight:600}}>Reject</button>
              </div>
            )}
          </div>))}
    ...
    {tab==="active"&&(<div className="aihub_card">
      <DataTable columns={[
        ...
        {label:"",render:r=><button onClick={()=>revoke(r.request_id)} style={{padding:"4px 12px",borderRadius:6,border:"1px solid #ef444440",background:"#ef444414",color:"#ef4444",cursor:"pointer",fontSize:14.1,fontWeight:600}}>Revoke</button>},
...
// Language identifiers are stored lowercase (they're compared/filtered on), but
// displayed with their conventional capitalization — plain capitalize-first
// would wrongly give "Javascript"/"Typescript" instead of "JavaScript"/"TypeScript".
const LANGUAGE_LABELS = { javascript: "JavaScript", typescript: "TypeScript", python: "Python", java: "Java", go: "Go", other: "Other" };
  ...
  if (import.meta.env.DEV) {
    return (<div className="aihub_card" style={{borderLeft:"3px solid #f59e0b"}}>
      <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
        ...
        <div>
          <div style={{fontWeight:700,fontSize:15.8,marginBottom:4}}>Admin credential required</div>
          <p className="aihub_text_muted" style={{fontSize:14.7,lineHeight:1.65,margin:0}}>
          ...
          <pre className="aihub_content_pre" style={{marginTop:10,fontSize:14.7}}>VITE_ADMIN_TOKEN=dev-admin-token</pre>
          <p className="aihub_text_muted" style={{fontSize:14.1,lineHeight:1.6,marginTop:8,marginBottom:0}}>
...
function sdkStep(n, title, body) {
  return (<li style={{display:"flex",gap:12,alignItems:"flex-start"}}>
    <span style={{flexShrink:0,width:24,height:24,borderRadius:"50%",background:"#0052e0",color:"#fff",fontSize:14.7,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",marginTop:1}}>{n}</span>
    <div style={{minWidth:0,flex:1}}>
      <div style={{fontSize:14.7,fontWeight:700,color:"#374151",marginBottom:6}}>{title}</div>
      {body}
    </div>
  </li>);
}
...
function SdkProjectsView() {
  ...
  const copyBtn=(key,text)=>(
    <button type="button" onClick={()=>{copyText(text);setCopied(key);setTimeout(()=>setCopied(null),1500);}}
      ...
      style={{display:"inline-flex",alignItems:"center",gap:5,padding:"4px 10px",borderRadius:6,fontSize:14.1,fontWeight:600,fontFamily:"inherit",cursor:"pointer",border:"1px solid #d1d5db",background:"#fff",color:copied===key?"#16a34a":"#374151"}}>
  ...
  return (<div>
    <SectionHeader title="SDK Projects" hint="Credentials for apps that report their AI activity to this server."
      action={<button type="button" onClick={()=>{setShowForm(s=>!s);setFormErr(null);}}
        style={{display:"inline-flex",alignItems:"center",gap:6,padding:"7px 14px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",fontSize:14.7,fontWeight:600,fontFamily:"inherit",cursor:"pointer"}}>
    ...
    {reveal&&(<div className="aihub_card" style={{border:"1px solid #f59e0b",background:"#fffbeb"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}}>
        <div style={{display:"flex",gap:10,minWidth:0}}>
          ...
          <div style={{minWidth:0}}>
            <div style={{fontWeight:800,fontSize:15.8,color:"#92400e"}}>Copy the secret key now — it will never be shown again</div>
            <p style={{fontSize:14.7,color:"#92400e",lineHeight:1.6,margin:"4px 0 0"}}>
        ...
        <button type="button" onClick={()=>setReveal(null)} title="Dismiss — the secret is discarded"
          style={{border:"1px solid #d1d5db",background:"#fff",borderRadius:6,padding:"4px 10px",fontSize:14.1,fontWeight:600,fontFamily:"inherit",cursor:"pointer",flexShrink:0}}>
          I have saved it
        </button>
      </div>
      <div style={{marginTop:14}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
          <span style={{fontSize:14.1,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:.4}}>Public key</span>
          {copyBtn("pk",reveal.public_key||"")}
        </div>
        <pre className="aihub_content_pre" style={{fontSize:14.7,padding:12,margin:0}}>{reveal.public_key}</pre>
        <div style={{display:"flex",alignItems:"center",gap:8,margin:"12px 0 4px"}}>
          <span style={{fontSize:14.1,fontWeight:700,color:"#b45309",textTransform:"uppercase",letterSpacing:.4}}>Secret key — shown once</span>
          {copyBtn("sk",reveal.secret_key||"")}
        </div>
        <pre className="aihub_content_pre" style={{fontSize:14.7,padding:12,margin:0,borderColor:"#fbbf24",background:"#fffbeb"}}>{reveal.secret_key}</pre>
      </div>
    </div>)}
    ...
    {showForm&&(<form className="aihub_card" onSubmit={create}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12}}>
        <label style={{display:"block"}}>
          <span style={{fontSize:14.1,fontWeight:600,color:"#374151"}}>Name <span style={{color:"#ef4444"}}>*</span></span>
          <input value={name} onChange={e=>setName(e.target.value)} required placeholder="checkout-assistant"
            style={{width:"100%",marginTop:4,padding:"7px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,boxSizing:"border-box"}}/>
        </label>
        <label style={{display:"block"}}>
          <span style={{fontSize:14.1,fontWeight:600,color:"#374151"}}>Language</span>
          <select value={lang} onChange={e=>setLang(e.target.value)}
            style={{width:"100%",marginTop:4,padding:"7px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,boxSizing:"border-box",background:"#fff"}}>
            {SDK_LANGS.map(l=><option key={l} value={l}>{languageLabel(l)}</option>)}
          </select>
        </label>
      </div>
      <label style={{display:"block",marginTop:12}}>
        <span style={{fontSize:14.1,fontWeight:600,color:"#374151"}}>Description</span>
      ...
      <div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:14}}>
        <button type="button" onClick={()=>{setShowForm(false);setFormErr(null);}}
          style={{padding:"7px 16px",borderRadius:8,border:"1px solid #e5e7eb",background:"#fff",fontSize:14.7,fontFamily:"inherit",cursor:"pointer"}}>Cancel</button>
        <button type="submit" disabled={busy||!name.trim()}
          style={{padding:"7px 16px",borderRadius:8,border:"none",background:busy||!name.trim()?"#9ca3af":"#0052e0",color:"#fff",fontSize:14.7,fontWeight:600,fontFamily:"inherit",cursor:busy||!name.trim()?"default":"pointer"}}>
    ...
    <div className="aihub_card">
      ...
      <ol role="list" style={{listStyle:"none",margin:"4px 0 0",padding:0,display:"flex",flexDirection:"column",gap:20}}>
        ...
        {sdkStep(1,"Download the SDK",<>
          ...
          <p className="aihub_text_muted" style={{fontSize:14.1,lineHeight:1.6,margin:"8px 0 0"}}>
        ...
        {sdkStep(2,"Put the folder in your project",<>
          <p className="aihub_text_muted" style={{fontSize:14.7,lineHeight:1.65,margin:"0 0 8px"}}>
          ...
          <pre className="aihub_content_pre" style={{fontSize:14.7,padding:12,margin:0}}>{tree}</pre>
        </>)}

        {sdkStep(3,"Set these three values",<>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>{copyBtn("env",envBlock)}</div>
          <pre className="aihub_content_pre" style={{fontSize:14.7,padding:12,margin:0}}>{envBlock}</pre>
          {!reveal&&<p className="aihub_text_muted" style={{fontSize:14.1,lineHeight:1.6,margin:"8px 0 0"}}>
        ...
        {sdkStep(4,"Add the tracer to your code",<>
          ...
          <pre className="aihub_content_pre" style={{fontSize:14.7,padding:12,margin:0}}>{snippet}</pre>
          <p className="aihub_text_muted" style={{fontSize:14.1,lineHeight:1.6,margin:"8px 0 0"}}>
  ...
  return (<div>
    ...
    <div className="aihub_card">
      {projects===null?<Loading/>:projAuthFail?(<div>
        <div style={{fontSize:14.7,fontWeight:600,color:"#374151",marginBottom:6}}>Project ID</div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          <input value={manualId} onChange={e=>setManualId(e.target.value)} placeholder="Paste a project id"
            style={{flex:"1 1 320px",padding:"7px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2}}/>
          <button type="button" onClick={()=>selectProject(manualId.trim())}
            style={{padding:"7px 16px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",fontSize:14.7,fontWeight:600,fontFamily:"inherit",cursor:"pointer"}}>Load traces</button>
        </div>
        <p className="aihub_text_muted" style={{fontSize:14.1,lineHeight:1.6,margin:"8px 0 0"}}>
        ...
        <div style={{fontSize:14.7,fontWeight:600,color:"#374151",marginBottom:6}}>Project</div>
        <select value={projectId} onChange={e=>selectProject(e.target.value)}
          style={{minWidth:320,padding:"7px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,background:"#fff"}}>
        ...
        renderExpanded={r=>{
          ...
          return (<div style={{padding:"14px 16px"}}>
            {(r.input_preview||r.output_preview)&&(<div style={{marginBottom:12,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div><div style={{fontSize:13.6,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:.4,marginBottom:3}}>Input preview</div>
                <div className="aihub_text_muted" style={{fontSize:14.7,lineHeight:1.5}}>{r.input_preview||"—"}</div></div>
              <div><div style={{fontSize:13.6,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:.4,marginBottom:3}}>Output preview</div>
                <div className="aihub_text_muted" style={{fontSize:14.7,lineHeight:1.5}}>{r.output_preview||"—"}</div></div>
            </div>)}

            {obs.length===0
              ?<p className="aihub_text_muted" style={{fontSize:14.7,margin:0}}>This trace has no recorded steps — the app opened it but never reported a span or generation.</p>
              :(<div>
                <div style={{fontSize:14.1,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:.4,marginBottom:6}}>
                ...
                {obs.map(o=>{
                  ...
                  return (<div key={o.id}>
                    <div role="button" tabIndex={0} onClick={()=>pickObs(o)}
                      ...
                      <div style={{flex:"0 0 210px",minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontSize:14.7}}>
                        <span style={{fontWeight:600}}>{o.name||o.type||"step"}</span>
                        <span className="aihub_text_muted" style={{marginLeft:6,fontSize:13.6}}>{String(o.type||"SPAN").toLowerCase()}</span>
                      </div>
                      ...
                      <div style={{flex:"0 0 66px",textAlign:"right",fontSize:14.1}} className="aihub_text_muted">{dur?`${Math.round(dur)}ms`:"—"}</div>
                      <div style={{flex:"0 0 74px",textAlign:"right"}}>{levelBadge(o.level)}</div>
                    </div>
                    {o.parent_cycle&&<div style={{marginLeft:14,fontSize:14.1,color:"#b45309"}}>Parent chain is circular — nesting shown flat.</div>}
                  </div>);
                })}
                {detail.truncated&&<p className="aihub_text_muted" style={{fontSize:14.1,marginTop:8}}>Only the first 2,000 steps of this trace are shown.</p>}
              </div>)}

            {active&&(<div style={{marginTop:12,padding:12,border:"1px solid #e5e7eb",borderRadius:10,background:"#fff"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:8}}>
                <div style={{fontSize:15.2,fontWeight:700}}>{active.name||active.type||"step"}</div>
                <div style={{display:"flex",gap:6,alignItems:"center"}}>
                  <Tag text={String(active.type||"SPAN").toLowerCase()} color={obsTone(active.type)}/>
                  {levelBadge(active.level)}
                </div>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:10,fontSize:14.7}}>
                <div><div className="aihub_text_muted" style={{fontSize:13.6}}>Model</div><Mono>{active.model||"—"}</Mono></div>
                <div><div className="aihub_text_muted" style={{fontSize:13.6}}>Latency</div>{active.latency_ms==null?"—":`${Math.round(active.latency_ms)}ms`}</div>
                <div><div className="aihub_text_muted" style={{fontSize:13.6}}>Usage</div>
                  {active.usage_details
                    ?<div style={{fontSize:14.7}}>{fmtTokens(active.usage_details.input)} in · {fmtTokens(active.usage_details.output)} out
                       <span className="aihub_text_muted"> ({fmtTokens(active.usage_details.total)} total)</span></div>
                    :<Mono>—</Mono>}</div>
                <div><div className="aihub_text_muted" style={{fontSize:13.6}}>Cost{active.cost_estimated?" (est.)":""}</div>
                  {active.cost_details
                    ?<div style={{fontSize:14.7}}>{fmtUsd(active.cost_details.total)}
                       <span className="aihub_text_muted"> ({fmtUsd(active.cost_details.input)} in / {fmtUsd(active.cost_details.output)} out)</span></div>
                    :<Mono>—</Mono>}</div>
              </div>
              {active.status_message&&<p style={{fontSize:14.7,color:"#b91c1c",margin:"8px 0 0"}}>{active.status_message}</p>}
              <div style={{marginTop:10,display:"flex",gap:8,alignItems:"center"}}>
                {active.has_io
                  ?<button type="button" onClick={()=>loadIo(active)} disabled={io?.status==="loading"}
                     style={{display:"inline-flex",alignItems:"center",gap:6,padding:"5px 12px",borderRadius:6,border:"1px solid #d1d5db",background:"#fff",fontSize:14.1,fontWeight:600,fontFamily:"inherit",cursor:"pointer"}}>
                     <Eye size={12}/>{io?.status==="loading"?"Loading…":"View content"}</button>
                  :<span className="aihub_text_muted" style={{fontSize:14.1}}>No raw content stored for this step — previews only.</span>}
              </div>
              {io?.status==="error"&&<div style={{marginTop:8}}><Err msg={io.message}/></div>}
              {io?.status==="ok"&&(<div style={{marginTop:10}}>
                <div style={{fontSize:13.6,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:.4,marginBottom:3}}>Input</div>
                <pre className="aihub_content_pre" style={{fontSize:14.7,padding:10,margin:0}}>{typeof io.input==="string"?io.input:JSON.stringify(io.input,null,2)||"—"}</pre>
                <div style={{fontSize:13.6,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:.4,margin:"10px 0 3px"}}>Output</div>
                <pre className="aihub_content_pre" style={{fontSize:14.7,padding:10,margin:0}}>{typeof io.output==="string"?io.output:JSON.stringify(io.output,null,2)||"—"}</pre>
              </div>)}
            </div>)}
          </div>);
        }}
...
function ClaudeUsageView() {
  ...
  const periodPicker=(
    <select value={days} onChange={ev=>setDays(ev.target.value)}
            style={{padding:"6px 10px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:14.7,fontWeight:600}}>
      ...
      {selected?.clients?.length>0 && (
        <div className="aihub_card" style={{marginBottom:14}}>
          ...
          {selected.clients.some(c=>c.client==="Unknown") && (
            <div className="aihub_text_muted" style={{marginTop:10,fontSize:13.4}}>
    ...
    <div>
      ...
      <DataTable
        columns={[
          ...
          { label: "Platform", render: r => <div><div className="aihub_text_primary">{r.product || r.ai_service}</div>{r.vendor && <div className="aihub_text_muted" style={{ fontSize:14.1 }}>{r.vendor}</div>}</div> },
          { label: "Prompts", key: "prompts", right: true },
      ...
      {platforms.length > 0 && (<>
        ...
        {platforms.map(p => (
          ...
          <div key={p.ai_service} style={{ marginBottom: 16 }}>
            <div style={{ fontWeight: 600, fontSize:15.2, marginBottom: 6 }}>{p.product || p.ai_service}</div>
            <DataTable
              columns={[
      ...
      <p className="aihub_text_muted" style={{ fontSize:14.1, marginTop: 8 }}>
  ...
  return (<div>
    ...
    {tab==="connections"&&(
      <div>
          ...
          {(connections||[]).map(c=>(
            <div key={c.type} style={{border:"1px solid "+(c.status==='configured'?"#22c55e30":"#e2e5ea"),borderRadius:12,padding:18,background:c.status==='configured'?"#f0fdf4":"#fff"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                <span style={{fontSize:33.6}}>{c.icon}</span>
                <div>
                  <div style={{fontWeight:700,fontSize:16.9}}>{c.name}</div>
                  <div style={{fontSize:14.7,color:"#9ca3af"}}>{c.description}</div>
                </div>
              </div>
              {c.status==='configured'?(
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",background:"#22c55e0a",borderRadius:8,border:"1px solid #22c55e20"}}>
                  <span style={{fontSize:14.7,fontWeight:700,color:"#22c55e"}}>✓ Connected</span>
                  <button onClick={()=>disconnect(c.type)} style={{fontSize:14.1,color:"#ef4444",background:"none",border:"none",cursor:"pointer"}}>Disconnect</button>
                </div>
              ):(
                <button onClick={()=>setConfigType(c.type)} style={{width:"100%",padding:"8px 0",borderRadius:8,border:"1px solid #0044cc30",background:"#0044cc08",color:"#0052e0",fontSize:15.2,fontWeight:600,cursor:"pointer"}}>Configure</button>
              )}
            </div>
          ))}
        </div>
        ...
        {configType==='slack'&&(
          <div className="aihub_card" style={{background:"#f5f6f8"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
              <h4 style={{margin:0,fontSize:15.8,fontWeight:700}}>Connect Slack</h4>
              <div style={{position:"relative"}}>
                <button onClick={()=>setShowHelp('slack')} style={{background:"none",border:"1px solid #0044cc30",borderRadius:"50%",width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#0052e0",fontWeight:700,fontSize:15.8}}>?</button>
                {!cfgSlackToken&&<div style={{position:"absolute",right:36,top:4,whiteSpace:"nowrap",fontSize:14.1,color:"#0052e0",fontWeight:600,animation:"cfai-fade-in .3s"}}>Need help? Click here →</div>}
              </div>
            </div>
            <div style={{marginBottom:12}}>
              <label style={{fontSize:14.7,fontWeight:600,color:"#374151",display:"block",marginBottom:4}}>Bot Token</label>
              <input value={cfgSlackToken} onChange={e=>setCfgSlackToken(e.target.value)} placeholder="xoxb-..." style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,boxSizing:"border-box"}}/>
            </div>
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
              <button onClick={()=>setConfigType(null)} style={{padding:"8px 20px",borderRadius:8,border:"1px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:15.2}}>Cancel</button>
              <button onClick={()=>saveConnection('slack')} disabled={!cfgSlackToken||cfgSaving} style={{padding:"8px 20px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:15.2,fontWeight:600,opacity:!cfgSlackToken||cfgSaving?0.5:1}}>{cfgSaving?"Verifying...":"Connect"}</button>
            </div>
          </div>
        )}
        ...
        {configType==='teams'&&(
          <div className="aihub_card" style={{background:"#f5f6f8"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
              <h4 style={{margin:0,fontSize:15.8,fontWeight:700}}>Connect Microsoft Teams</h4>
              <div style={{position:"relative"}}>
                <button onClick={()=>setShowHelp('teams')} style={{background:"none",border:"1px solid #0044cc30",borderRadius:"50%",width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#0052e0",fontWeight:700,fontSize:15.8}}>?</button>
                {!cfgTeamsClientId&&<div style={{position:"absolute",right:36,top:4,whiteSpace:"nowrap",fontSize:14.1,color:"#0052e0",fontWeight:600,animation:"cfai-fade-in .3s"}}>Need help? Click here →</div>}
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
              <div>
                <label style={{fontSize:14.7,fontWeight:600,color:"#374151",display:"block",marginBottom:4}}>Client ID</label>
                <input value={cfgTeamsClientId} onChange={e=>setCfgTeamsClientId(e.target.value)} placeholder="Azure App Client ID" style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,boxSizing:"border-box"}}/>
              </div>
              <div>
                <label style={{fontSize:14.7,fontWeight:600,color:"#374151",display:"block",marginBottom:4}}>Tenant ID</label>
                <input value={cfgTeamsTenant} onChange={e=>setCfgTeamsTenant(e.target.value)} placeholder="Azure Tenant ID" style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,boxSizing:"border-box"}}/>
              </div>
            </div>
            <div style={{marginBottom:12}}>
              <label style={{fontSize:14.7,fontWeight:600,color:"#374151",display:"block",marginBottom:4}}>Client Secret</label>
              <input type="password" value={cfgTeamsSecret} onChange={e=>setCfgTeamsSecret(e.target.value)} placeholder="Azure App Client Secret" style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,boxSizing:"border-box"}}/>
            </div>
            {teamsManualUpload&&(
              <div style={{marginBottom:12,padding:14,background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:10}}>
                <div style={{fontWeight:700,fontSize:15.2,color:"#92400e",marginBottom:6}}>⚠ Connected — but bot app needs manual upload</div>
                <div style={{fontSize:14.7,color:"#78350f",lineHeight:1.6,marginBottom:10}}>
                ...
                <a href={CONNECTIONS_API+"/teams/manifest"} download="CloudFuze-Alerts-Bot.zip"
                  style={{display:"inline-flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",fontSize:15.2,fontWeight:600,textDecoration:"none",cursor:"pointer"}}>
                  ⬇ Download Manifest ZIP
                </a>
              </div>
            )}
            <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
              <button onClick={()=>{setConfigType(null);setTeamsManualUpload(false);}} style={{padding:"8px 20px",borderRadius:8,border:"1px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:15.2}}>{teamsManualUpload?"Done":"Cancel"}</button>
              {!teamsManualUpload&&<button onClick={()=>saveConnection('teams')} disabled={!cfgTeamsClientId||!cfgTeamsSecret||!cfgTeamsTenant||cfgSaving} style={{padding:"8px 20px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:15.2,fontWeight:600,opacity:(!cfgTeamsClientId||!cfgTeamsSecret||!cfgTeamsTenant||cfgSaving)?0.5:1}}>{cfgSaving?"Verifying...":"Connect"}</button>}
            </div>
          </div>
        )}
      </div>
    ...
    {showHelp&&(
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowHelp(null)}>
        <div style={{background:"#fff",borderRadius:14,padding:28,width:640,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}} onClick={e=>e.stopPropagation()}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
            <h3 style={{margin:0,fontSize:19.7,fontWeight:700}}>{showHelp==='slack'?'How to Connect Slack':'How to Connect Microsoft Teams'}</h3>
            <button onClick={()=>setShowHelp(null)} style={{background:"none",border:"none",cursor:"pointer",fontSize:21.9,color:"#6b7280"}}>✕</button>
          </div>

          {showHelp==='slack'&&(<div style={{fontSize:15.2,color:"#374151",lineHeight:1.8}}>
            <div style={{fontWeight:700,fontSize:15.8,color:"#0052e0",marginBottom:8}}>Step 1 — Create a Slack App</div>
            ...
            <div style={{fontWeight:700,fontSize:15.8,color:"#0052e0",marginBottom:8}}>Step 2 — Add Bot Permissions</div>
            <ol style={{paddingLeft:20,marginBottom:16}}>
              ...
              <li>Click <strong>Add an OAuth Scope</strong> and add these:
                <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>chat:write</code>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>channels:read</code>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>channels:join</code>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>groups:read</code>
                </div>
              </li>
            </ol>
            <div style={{fontWeight:700,fontSize:15.8,color:"#0052e0",marginBottom:8}}>Step 3 — Install & Copy Token</div>
            <ol style={{paddingLeft:20,marginBottom:16}}>
            ...
            <div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,padding:12,fontSize:14.7,color:"#166534"}}>
          ...
          {showHelp==='teams'&&(<div style={{fontSize:15.2,color:"#374151",lineHeight:1.8}}>
            <div style={{fontWeight:700,fontSize:15.8,color:"#0052e0",marginBottom:8}}>Step 1 — Create Azure Bot Resource</div>
            <ol style={{paddingLeft:20,marginBottom:16}}>
            ...
            <div style={{fontWeight:700,fontSize:15.8,color:"#0052e0",marginBottom:8}}>Step 2 — Get App ID & Secret</div>
            <ol style={{paddingLeft:20,marginBottom:16}}>
            ...
            <div style={{fontWeight:700,fontSize:15.8,color:"#0052e0",marginBottom:8}}>Step 3 — Enable Teams Channel</div>
            <ol style={{paddingLeft:20,marginBottom:16}}>
            ...
            <div style={{fontWeight:700,fontSize:15.8,color:"#0052e0",marginBottom:8}}>Step 4 — Add API Permissions</div>
            <ol style={{paddingLeft:20,marginBottom:16}}>
              ...
              <li>Add these permissions:
                <div style={{display:"flex",gap:6,marginTop:6,flexWrap:"wrap"}}>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>Group.Read.All</code>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>Team.ReadBasic.All</code>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>Channel.ReadBasic.All</code>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>AppCatalog.Read.All</code>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>AppCatalog.ReadWrite.All</code>
                  <code style={{background:"#dbeafe",padding:"2px 8px",borderRadius:4,fontSize:14.7}}>TeamsAppInstallation.ReadWriteForTeam.All</code>
                </div>
              </li>
              <li>Click <strong>Grant admin consent for [your org]</strong></li>
            </ol>
            <div style={{fontWeight:700,fontSize:15.8,color:"#0052e0",marginBottom:8}}>Step 5 — Connect in CloudFuze</div>
            <ol style={{paddingLeft:20,marginBottom:16}}>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",marginTop:16}}>
            <button onClick={()=>setShowHelp(null)} style={{padding:"8px 24px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:15.2,fontWeight:600}}>Got it</button>
          </div>
        </div>
      </div>
    )}
    ...
    {tab==="webhooks"&&(<div>
      <div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}>
        ...
        }} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:15.2,fontWeight:600}}><Plus size={14}/> Add Webhook</button>
      </div>
      {showForm&&(<div className="aihub_card" style={{marginBottom:16,background:"#f5f6f8"}}>
        <h4 style={{margin:"0 0 12px",fontSize:15.8,fontWeight:700}}>{editId?"Edit Webhook":"New Webhook"}</h4>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <div><label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Name</label><input value={fName} onChange={e=>setFName(e.target.value)} placeholder="e.g. Security Alerts Slack" style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,marginTop:4,boxSizing:"border-box"}}/></div>
          <div><label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Send To</label><select value={fTemplate} onChange={e=>{setFTemplate(e.target.value);setFChannelId("");setChannelData(null);setChannelSearch("");if(e.target.value==='slack'||e.target.value==='teams')loadChannels(e.target.value);}} style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,marginTop:4,boxSizing:"border-box"}}>
        ...
        {(fTemplate==='slack'||fTemplate==='teams')&&(
          <div style={{marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
              <label style={{fontSize:14.7,fontWeight:600,color:"#374151"}}>Channel {fChannelId&&<span style={{fontWeight:400,color:"#22c55e"}}> ✓ selected</span>}</label>
              <button type="button" onClick={()=>loadChannels(fTemplate)} disabled={loadingChannels} style={{fontSize:14.1,color:"#0052e0",background:"none",border:"none",cursor:"pointer",fontWeight:600}}>{loadingChannels?"Loading...":"↻ Refresh"}</button>
            </div>
            {loadingChannels?(
              <div style={{padding:"16px",background:"#f5f6f8",borderRadius:8,fontSize:14.7,color:"#6b7280",textAlign:"center"}}>Loading from {fTemplate==='slack'?'Slack':'Microsoft Teams'}...</div>
            ):channelData?(()=>{
              ...
              if(channelData.type==='hierarchical'){
                ...
                return (<div style={{border:"1px solid #e5e7eb",borderRadius:8,overflow:"hidden"}}>
                  <div style={{padding:"8px 10px",borderBottom:"1px solid #e5e7eb",background:"#f5f6f8"}}>
                    <input value={channelSearch} onChange={e=>setChannelSearch(e.target.value)} placeholder={"Search "+filteredTeams.length+" teams..."} style={{width:"100%",border:"none",background:"transparent",outline:"none",fontSize:14.7,boxSizing:"border-box"}}/>
                  </div>
                  <div style={{maxHeight:300,overflowY:"auto"}}>
                    {filteredTeams.length===0&&<div style={{padding:12,textAlign:"center",fontSize:14.7,color:"#9ca3af"}}>No teams match "{channelSearch}"</div>}
                    {filteredTeams.map(t=>(
                      <div key={t.team_id}>
                        <div onClick={()=>expandTeam(t.team_id)} style={{padding:"8px 12px",fontSize:14.7,fontWeight:600,color:"#374151",background:expandedTeam===t.team_id?"#eef2ff":"#f9fafb",borderBottom:"1px solid #f3f4f6",cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
                          <span style={{fontSize:13,color:"#6b7280"}}>{expandedTeam===t.team_id?"▼":"▶"}</span> 🏢 {t.team_name}
                        </div>
                        {expandedTeam===t.team_id&&(
                          loadingTeamCh===t.team_id?
                            <div style={{padding:"8px 32px",fontSize:14.1,color:"#9ca3af"}}>Loading channels...</div>
                          :(teamChannels[t.team_id]||[]).map(ch=>(
                            <div key={ch.id} onClick={()=>setFChannelId(ch.id)}
                              style={{padding:"7px 12px 7px 36px",fontSize:14.7,cursor:"pointer",borderBottom:"1px solid #f9fafb",
                ...
                return (<div style={{border:"1px solid #e5e7eb",borderRadius:8,overflow:"hidden"}}>
                  <div style={{padding:"8px 10px",borderBottom:"1px solid #e5e7eb",background:"#f5f6f8"}}>
                    <input value={channelSearch} onChange={e=>setChannelSearch(e.target.value)} placeholder={"Search "+chs.length+" channels..."} style={{width:"100%",border:"none",background:"transparent",outline:"none",fontSize:14.7,boxSizing:"border-box"}}/>
                  </div>
                  <div style={{maxHeight:300,overflowY:"auto"}}>
                    {chs.length===0&&<div style={{padding:12,textAlign:"center",fontSize:14.7,color:"#9ca3af"}}>No channels match "{channelSearch}"</div>}
                    {chs.map(ch=>(
                      <div key={ch.id} onClick={()=>setFChannelId(ch.id)}
                        style={{padding:"7px 12px",fontSize:14.7,cursor:"pointer",borderBottom:"1px solid #f9fafb",
                          background:fChannelId===ch.id?"#0044cc10":"#fff",color:fChannelId===ch.id?"#0052e0":"#374151",fontWeight:fChannelId===ch.id?600:400}}>
                        {ch.name} {ch.is_private&&<span style={{fontSize:13,color:"#9ca3af"}}>🔒</span>}
                      </div>
                    ))}
                  </div>
                </div>);
              ...
              <div style={{padding:"10px 12px",background:"#fff7ed",borderRadius:8,fontSize:14.7,color:"#92400e",border:"1px solid #fed7aa"}}>Click "↻ Refresh" to load from {fTemplate==='slack'?'Slack':'Microsoft Teams'}</div>
            )}
          </div>
        )}
        ...
        {fTemplate==='custom'&&(<>
          <div style={{marginBottom:12}}><label style={{fontSize:14.7,fontWeight:600,color:"#374151",display:"block",marginBottom:4}}>Webhook URL</label><input value={fUrl} onChange={e=>setFUrl(e.target.value)} placeholder="https://..." style={{width:"100%",padding:"8px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:15.2,boxSizing:"border-box"}}/></div>
        ...
        <div style={{marginBottom:12}}><label style={{fontSize:14.7,fontWeight:600,color:"#374151",marginBottom:6,display:"block"}}>Triggers</label><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{triggersList.map(t=>{const sel=fTriggers.includes(t);return <button key={t} type="button" onClick={()=>setFTriggers(sel?fTriggers.filter(x=>x!==t):[...fTriggers,t])} style={{padding:"4px 12px",borderRadius:6,fontSize:14.1,fontWeight:600,border:"1px solid",cursor:"pointer",background:sel?"#0044cc14":"#fff",color:sel?"#0052e0":"#6b7280",borderColor:sel?"#0044cc30":"#e2e5ea"}}>{TL[t]||t}</button>;})}</div></div>
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><button onClick={closeForm} style={{padding:"8px 20px",borderRadius:8,border:"1px solid #e5e7eb",background:"#fff",cursor:"pointer",fontSize:15.2}}>Cancel</button><button onClick={save} disabled={!fName} style={{padding:"8px 20px",borderRadius:8,border:"none",background:"#0052e0",color:"#fff",cursor:"pointer",fontSize:15.2,fontWeight:600,opacity:!fName?0.5:1}}>{editId?"Update":"Save"}</button></div>
      </div>)}
      <div className="aihub_card"><DataTable columns={[
        {label:"Webhook",render:r=><div><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:18}}>{TI[r.template]||"⚡"}</span><div className="aihub_text_primary">{r.name}</div></div><div className="aihub_text_muted" style={{fontSize:14.1}}>{r.url?.slice(0,50)}{r.url?.length>50?"...":""}</div></div>},
        ...
        {label:"Actions",render:r=><div style={{display:"flex",gap:6,whiteSpace:"nowrap"}}><button onClick={()=>startEdit(r)} style={{background:"none",border:"none",cursor:"pointer",color:"#0052e0",fontSize:14.7,fontWeight:600}}>Edit</button><button onClick={()=>test(r.id)} disabled={testing===r.id} style={{background:"none",border:"none",cursor:"pointer",color:"#6366f1",fontSize:14.7,fontWeight:600}}>{testing===r.id?"Testing...":"Test"}</button><button onClick={()=>toggle(r)} style={{background:"none",border:"none",cursor:"pointer",color:"#f59e0b",fontSize:14.7,fontWeight:600}}>{r.enabled?"Disable":"Enable"}</button><button onClick={()=>remove(r.id)} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",fontSize:14.7,fontWeight:600}}>Delete</button></div>},
    ...
    {tab==="log"&&(<div className="aihub_card"><DataTable columns={[
      ...
      {label:"Status",render:r=><Badge text={capitalizeWord(r.status)} color={r.status==='delivered'?"#22c55e":r.status==='failed'?"#f59e0b":"#ef4444"}/>},
      {label:"HTTP",render:r=>r.http_status||"—"},
      {label:"Error",render:r=><div className="aihub_text_muted" style={{fontSize:14.1,maxWidth:200,overflow:"hidden",textOverflow:"ellipsis"}}>{r.error||"—"}</div>},
      ...
      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>{setOpenId(null);setDetail(null);}}>
        <div style={{background:"#fff",borderRadius:14,padding:28,width:900,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}} onClick={e=>e.stopPropagation()}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
            <div>
              <h3 style={{margin:"0 0 4px",fontSize:19.7,fontWeight:700}}>{detail.framework} — {detail.ruleCount} rules</h3>
            ...
            <div style={{display:"flex",gap:6}}>
              ...
              <button onClick={()=>{setOpenId(null);setDetail(null);}} style={{background:"none",border:"none",cursor:"pointer",fontSize:21.9,color:"#6b7280"}}>✕</button>
            </div>
          </div>

          <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:12,fontSize:14.1}}>
          ...
          <DataTable columns={[
            ...
            {label:"",render:r=>{
              if(!detail.deployed) return <span className="aihub_text_muted" style={{fontSize:14.1}}>deploy first</span>;
              if(r.enforcement==="agent") return <div style={{display:"flex",gap:6,alignItems:"center"}}>
                ...
                {r.tunable && <input type="number" defaultValue={r.tuned_value ?? r.conditions?.[0]?.value}
                  ...
                  style={{width:70,padding:"3px 6px",fontSize:14.7,border:"1px solid #e5e7eb",borderRadius:4}}
              ...
              return <span className="aihub_text_muted" style={{fontSize:14.1}}>{(r.patterns||[]).length} patterns</span>;
            }},
...
function PackSimulation({ pack, onClose }) {
  ...
  return (<div className="aihub_card" style={{marginBottom:18,borderLeft:"3px solid #0044cc"}}>
    ...
    {pack.monitored===0
      ? <p className="aihub_text_muted" style={{fontSize:14.7,margin:0}}>
      ...
      : <>
        <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10,flexWrap:"wrap"}}>
          <span style={{fontSize:14.7,fontWeight:600}}>Window</span>
          {[7,30,90].map(d=>(
            <button key={d} className={`aihub_filter_btn ${days===d?"active":""}`} disabled={busy} onClick={()=>setDays(d)}>{d} days</button>
          ))}
          {opts && <span className="aihub_text_muted" style={{fontSize:14.1}}>{opts.events_available} prompt events captured in this window</span>}
        </div>

        {!!opts?.services?.length && <div style={{marginBottom:12}}>
          <div style={{fontSize:14.7,fontWeight:600,marginBottom:5}}>Allow these tools (exclude from blocking)</div>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:result?14:0}}>
          ...
          <label style={{fontSize:14.7,display:"flex",alignItems:"center",gap:5}}>
    ...
    {result && <>
      ...
      {!!result.samples?.length && <div style={{...SIM_PANEL,marginBottom:14}}>
        ...
        <DataTable columns={[
          ...
          {label:"Excerpt",render:r=>r.excerpt_available?<span style={{fontSize:14.1}}>{r.excerpt}</span>:<span className="aihub_text_muted">no text retained</span>},
        ]} rows={result.samples} empty="No excerpts available."/>
      </div>}

      <div style={SIM_PANEL}>
        <SectionHeader title={pack.deployed?"Worth knowing":"Before you deploy"}/>
        <ul style={{margin:0,paddingLeft:18,fontSize:14.7,color:"#4b5563"}}>
  ...
  return (<div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
      ...
      <input placeholder="Your name (compliance officer)" value={officer} onChange={e=>setOfficer(e.target.value)}
             style={{padding:"5px 10px",fontSize:14.7,border:"1px solid #e5e7eb",borderRadius:6,minWidth:210}}/>
    </div>
    ...
    {mode==="wizard" && <div className="aihub_card">
      ...
      <input placeholder="AI system name (e.g. HR CV Screener)" value={sysName} onChange={e=>setSysName(e.target.value)}
             style={{padding:"7px 11px",fontSize:15.2,border:"1px solid #e5e7eb",borderRadius:6,width:"100%",maxWidth:460,marginBottom:14}}/>
      ...
      {["prohibited","high_risk","transparency","context"].map(sec=>{
        ...
        return (<div key={sec} style={{marginBottom:16}}>
          <div style={{fontSize:14.7,fontWeight:700,color:"#0052e0",marginBottom:6}}>{title}</div>
          {qs.map(q=>(
            <div key={q.id} style={{padding:"8px 0",borderBottom:"1px solid #f1f5f9"}}>
              <div style={{fontSize:14.7,marginBottom:3}}>{q.question}</div>
              <div className="aihub_text_muted" style={{fontSize:14.1,marginBottom:5}}>{q.help} · <Mono>{q.citation}</Mono></div>
              <div style={{display:"flex",gap:6}}>
      ...
      {live && <div style={{padding:"12px 14px",borderRadius:6,marginBottom:12,
        ...
        <div style={{fontSize:15.2,fontWeight:700,color:TIER_COLOR[live.tier]}}>Proposed: {live.tier_meta.label}</div>
        <div style={{fontSize:14.7,margin:"4px 0 6px"}}>{live.tier_meta.summary}</div>
        {!!live.reasons.length && <div style={{fontSize:14.1,color:"#4b5563"}}>Because: {live.reasons.map(r=>r.citation).join(", ")}</div>}
        {live.fria_required && <div style={{fontSize:14.1,color:"#b45309",marginTop:4}}><strong>A FRIA is required</strong> (Article 27) — you indicated a public body or essential-service operator.</div>}
        <div style={{fontSize:14.1,marginTop:8}}><strong>Obligations if confirmed:</strong></div>
        <ul style={{margin:"3px 0 0 16px",fontSize:14.1,color:"#4b5563"}}>
          {live.tier_meta.obligations.map((o,i)=><li key={i}>{o}</li>)}
        </ul>
      </div>}

      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:10}}>
        <span style={{fontSize:14.7,fontWeight:600}}>Override tier (optional)</span>
      ...
      {ovTier && <input placeholder="Why are you overriding the proposed tier? (required, recorded in the report)"
              ...
              style={{padding:"7px 11px",fontSize:14.7,border:"1px solid #e5e7eb",borderRadius:6,width:"100%",marginBottom:10}}/>}

      <button className="aihub_action_btn" disabled={busy||!sysName||!officer||(!!ovTier&&!ovWhy)} onClick={save}>
        {busy?"Saving…":"Save classification"}
      </button>
      {(!sysName||!officer) && <div className="aihub_text_muted" style={{fontSize:14.1,marginTop:5}}>
    ...
    {mode==="fria" && friaFor && <div className="aihub_card">
      ...
      {meta.fria_questions.map(q=>(
        <div key={q.id} style={{marginBottom:12}}>
          <div style={{fontSize:14.7,fontWeight:600}}>{q.prompt}</div>
          <div className="aihub_text_muted" style={{fontSize:14.1,marginBottom:4}}>{q.help} · <Mono>{q.citation}</Mono></div>
          <textarea rows={q.kind==="list"?2:3} value={friaAns[q.id]||""}
                    onChange={e=>setFriaAns(a=>({...a,[q.id]:e.target.value}))}
                    style={{width:"100%",padding:"7px 10px",fontSize:14.7,border:"1px solid #e5e7eb",
      ...
      <div className="aihub_text_muted" style={{fontSize:14.1,marginTop:5}}>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 2, marginBottom: 16, borderBottom: "1px solid #e5e7eb", paddingBottom: 0 }}>
        {smTabs.map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); setSelectedTrace(null); }}
            style={{ display: "flex", alignItems: "center", gap: 5, padding: "8px 16px", border: "none", background: "none", cursor: "pointer", fontSize:15.2, fontWeight: tab === t.id ? 600 : 400, color: tab === t.id ? "#2563eb" : "#6b7280", borderBottom: tab === t.id ? "2px solid #2563eb" : "2px solid transparent", marginBottom: -1 }}>
            {t.icon} {t.label}
          </button>
        ))}
      </div>
                  ...
                  <div key={srv.machine_id} onClick={() => setSelectedServer(srv)}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 600, fontSize:16.9 }}>{srv.display_name || srv.hostname || srv.machine_id?.slice(0, 12)}</div>
                          <div className="aihub_text_muted" style={{ fontSize:14.7 }}>{srvGoverned.length} agent{srvGoverned.length !== 1 ? "s" : ""} governed | {srv.total_calls || 0} traces | {fmtUsd(srv.total_cost_usd)} | avg {srv.avg_latency_ms ? (srv.avg_latency_ms / 1000).toFixed(1) + "s" : "--"}</div>
                        </div>
                      </div>
      ...
      {tab === "traces" && selectedServer && !selectedAgent && (
        <div>
          <button onClick={() => setSelectedServer(null)} style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", fontSize:15.2, marginBottom: 12, padding: 0, display: "flex", alignItems: "center", gap: 4 }}>
                  ...
                  <div key={agent.container_name} onClick={() => { setSelectedAgent(agent); setDateFrom(""); setDateTo(""); }}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 600, fontSize:15.8 }}>{agent.container_name}</div>
                          <div className="aihub_text_muted" style={{ fontSize:14.7 }}>
      ...
      {tab === "traces" && selectedServer && selectedAgent && !selectedTrace && (
        <div>
          <button onClick={() => setSelectedAgent(null)} style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", fontSize:15.2, marginBottom: 12, padding: 0, display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 14, alignItems: "center", flexWrap: "wrap" }}>
            <label style={{ fontSize:14.7, color: "#6b7280" }}>From:</label>
            <input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); }} style={{ padding: "4px 8px", border: "1px solid #d1d5db", borderRadius: 6, fontSize:14.7 }} />
            <label style={{ fontSize:14.7, color: "#6b7280" }}>To:</label>
            <input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); }} style={{ padding: "4px 8px", border: "1px solid #d1d5db", borderRadius: 6, fontSize:14.7 }} />
            <button onClick={fetchAgentCalls} style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #d1d5db", background: "#fff", fontSize:14.7, cursor: "pointer" }}>Filter</button>
            {(dateFrom || dateTo) && <button onClick={() => { setDateFrom(""); setDateTo(""); }} style={{ padding: "4px 12px", borderRadius: 6, border: "none", background: "#f3f4f6", fontSize:14.7, cursor: "pointer", color: "#6b7280" }}>Clear</button>}
          </div>
          <div style={{ maxHeight: 600, overflowY: "auto" }}>
          <DataTable columns={[
            { label: "Time", render: r => <span style={{ fontSize:14.7 }}>{relTime(r.occurred_at)}</span> },
            { label: "Provider", render: r => <Tag text={r.provider || "?"} color={r.provider === "openai" ? "#10a37f" : r.provider === "anthropic" ? "#d97706" : "#6366f1"} /> },
            { label: "Model", render: r => <span style={{ fontSize:14.7, fontWeight: 500 }}>{(r.model || "?").replace(/-\d{4}-\d{2}-\d{2}$/, "")}</span> },
      ...
      {tab === "traces" && selectedTrace && traceDetail && (
        <div>
          <button onClick={() => setSelectedTrace(null)} style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", fontSize:15.2, marginBottom: 12, padding: 0, display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 24 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  ...
                  <h3 style={{ margin: 0, fontSize:18 }}>{(traceDetail.model || "?").replace(/-\d{4}-\d{2}-\d{2}$/, "")}</h3>
                </div>
                <div className="aihub_text_muted" style={{ fontSize:14.7 }}>
              ...
              <div style={{ display: "flex", gap: 16, textAlign: "right" }}>
                <div><div style={{ fontSize:19.7, fontWeight: 700 }}>{fmtTokens((traceDetail.prompt_tokens||0)+(traceDetail.completion_tokens||0))}</div><div className="aihub_text_muted" style={{ fontSize:14.1 }}>tokens</div></div>
                <div><div style={{ fontSize:19.7, fontWeight: 700 }}>{traceDetail.duration_ms ? (traceDetail.duration_ms/1000).toFixed(1)+"s" : "--"}</div><div className="aihub_text_muted" style={{ fontSize:14.1 }}>duration</div></div>
                <div><div style={{ fontSize:19.7, fontWeight: 700 }}>{fmtUsd(traceCost(traceDetail))}</div><div className="aihub_text_muted" style={{ fontSize:14.1 }}>cost</div></div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
              <div style={{ flex: 1, background: "#f0fdf4", borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize:14.1, color: "#166534", fontWeight: 600, marginBottom: 2 }}>PROMPT TOKENS</div>
                <div style={{ fontSize:21.9, fontWeight: 700, color: "#166534" }}>{fmtTokens(traceDetail.prompt_tokens)}</div>
              </div>
              <div style={{ flex: 1, background: "#eff6ff", borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize:14.1, color: "#1e40af", fontWeight: 600, marginBottom: 2 }}>COMPLETION TOKENS</div>
                <div style={{ fontSize:21.9, fontWeight: 700, color: "#1e40af" }}>{fmtTokens(traceDetail.completion_tokens)}</div>
              </div>
              <div style={{ flex: 1, background: "#fefce8", borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize:14.1, color: "#854d0e", fontWeight: 600, marginBottom: 2 }}>RESPONSE STATUS</div>
                <div style={{ fontSize:21.9, fontWeight: 700, color: traceDetail.response_status >= 400 ? "#dc2626" : "#166534" }}>{traceDetail.response_status || 200}</div>
              </div>
            </div>
              ...
              return (
                <>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize:14.7, fontWeight: 600, color: "#166534", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8, padding: 14 }}>
                      {userInput ? (
                        <div style={{ fontSize:15.8, lineHeight: 1.6, color: "#1f2937" }}>{userInput}</div>
                      ) : (
                        <span className="aihub_text_muted" style={{ fontSize:14.7 }}>User input not available.</span>
                      )}
                    </div>
                  </div>
                  ...
                  {toolCalls.length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize:14.7, fontWeight: 600, color: "#7c3aed", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      <div style={{ display: "grid", gap: 8 }}>
                        {toolCalls.slice(-5).map((tc, i) => (
                          <div key={i} style={{ background: "#f5f3ff", border: "1px solid #ddd6fe", borderRadius: 8, padding: 12 }}>
                            <div style={{ fontSize:15.2, fontWeight: 600, color: "#5b21b6", marginBottom: 4 }}>{tc.name || "unknown"}</div>
                            <pre style={{ margin: 0, fontSize:14.1, color: "#6b7280", whiteSpace: "pre-wrap", wordBreak: "break-word", maxHeight: 100, overflowY: "auto" }}>
                  ...
                  {/* Assistant Output */}
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize:14.7, fontWeight: 600, color: "#1e40af", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8, padding: 14 }}>
                      {assistantOutput ? (
                        <div style={{ fontSize:15.8, lineHeight: 1.6, color: "#1f2937", whiteSpace: "pre-wrap" }}>{assistantOutput}</div>
                      ) : (
                        <span className="aihub_text_muted" style={{ fontSize:14.7 }}>Assistant output not available.</span>
                      )}
                    </div>
                  </div>
                  ...
                  {systemPrompt && (
                    <details style={{ marginBottom: 16 }}>
                      <summary style={{ fontSize:14.7, fontWeight: 600, color: "#6b7280", cursor: "pointer", marginBottom: 6 }}>
                        System Prompt ({systemPrompt.length} chars)
                      </summary>
                      <div style={{ background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8, padding: 14, maxHeight: 200, overflowY: "auto" }}>
                        <pre style={{ margin: 0, fontSize:14.1, lineHeight: 1.5, whiteSpace: "pre-wrap", wordBreak: "break-word", color: "#6b7280" }}>
                          {systemPrompt}
                        </pre>
                      </div>
                    </details>
      ...
      {tab === "servers" && (
        <div>
          ...
          <DataTable columns={[
            ...
            { label: "Status", render: r => <Badge text={capitalizeWord(r.status)} color={r.status === "active" ? "#22c55e" : "#9ca3af"} /> },
            ...
            { label: "Providers", render: r => <div>{(r.providers || []).map(p => <Tag key={p} text={slugLabel(p)} />)}</div> },
        ...
        <div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, padding: 24 }}>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
              <div style={{ width: 32, height: 32, minWidth: 32, borderRadius: "50%", background: "#dbeafe", color: "#2563eb", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize:15.8 }}>1</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize:15.8, marginBottom: 4 }}>Configure your server</div>
                <div className="aihub_text_muted" style={{ fontSize:14.7, marginBottom: 14 }}>Enter the proxy port the monitor will listen on. This port intercepts all outbound AI API traffic from every process on the server.</div>

                <div style={{ display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
                  <div>
                    <label style={{ display: "block", fontSize:14.7, fontWeight: 600, color: "#374151", marginBottom: 4 }}>Proxy Port</label>
                    <input type="number" value={proxyPort} onChange={e => { setProxyPort(e.target.value); setInstallCmd(""); }}
                      style={{ width: 120, padding: "8px 12px", border: "1px solid #d1d5db", borderRadius: 6, fontSize:15.8, fontFamily: "ui-monospace, monospace" }}
                      placeholder="8443" min="1024" max="65535" />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize:14.7, fontWeight: 600, color: "#374151", marginBottom: 4 }}>Server IP / Hostname <span style={{ fontWeight: 400, color: "#9ca3af" }}>(optional)</span></label>
                    <input type="text" value={serverIp} onChange={e => { setServerIp(e.target.value); setInstallCmd(""); }}
                      style={{ width: 220, padding: "8px 12px", border: "1px solid #d1d5db", borderRadius: 6, fontSize:15.8 }}
                      placeholder="auto-detect" />
                  </div>
                  <button onClick={generateInstallCmd} disabled={generating || !proxyPort}
                    style={{ padding: "8px 20px", borderRadius: 6, border: "none", background: "#2563eb", color: "#fff", fontSize:15.2, fontWeight: 600, cursor: generating ? "wait" : "pointer", opacity: generating ? 0.6 : 1, height: 38 }}>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 24 }}>
              <div style={{ width: 32, height: 32, minWidth: 32, borderRadius: "50%", background: installCmd ? "#dcfce7" : "#f3f4f6", color: installCmd ? "#16a34a" : "#9ca3af", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize:15.8 }}>2</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize:15.8, marginBottom: 4 }}>Run this command on your server</div>
                <div className="aihub_text_muted" style={{ fontSize:14.7, marginBottom: 10 }}>SSH into your server and paste this command. Requires sudo access.</div>
                {installCmd ? (
                  <div style={{ background: "#1e293b", color: "#e2e8f0", borderRadius: 8, padding: 16, fontFamily: "ui-monospace, monospace", fontSize:14.7, overflowX: "auto", position: "relative", lineHeight: 1.6, wordBreak: "break-all" }}>
                  ...
                  <div style={{ background: "#f9fafb", border: "1px dashed #d1d5db", borderRadius: 8, padding: 16, textAlign: "center", color: "#9ca3af", fontSize:15.2 }}>
            ...
            {/* Step 3: Done */}
            <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
              <div style={{ width: 32, height: 32, minWidth: 32, borderRadius: "50%", background: "#f3f4f6", color: "#9ca3af", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize:15.8 }}>3</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize:15.8, marginBottom: 4 }}>ALL agents are instantly governed</div>
                <div className="aihub_text_muted" style={{ fontSize:14.7 }}>The installer sets up an <code style={{ background: "#f1f5f9", padding: "1px 4px", borderRadius: 3 }}>iptables</code> transparent redirect that captures ALL outbound port-443 traffic at the kernel level. Every process &mdash; already running or new &mdash; is governed instantly. No code changes, no env vars, no agent restarts needed. Works with Node.js, Python, Go, curl, anything.</div>
              </div>
            </div>

            {/* What gets captured */}
            <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 8, padding: 14, marginTop: 8 }}>
              <div style={{ fontWeight: 600, fontSize:15.2, color: "#166534", marginBottom: 6 }}>What gets captured from every AI agent on the server:</div>
              <ul style={{ margin: 0, paddingLeft: 18, fontSize:14.7, color: "#166534", lineHeight: 1.8 }}>
            ...
            <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8, padding: 14, marginTop: 12 }}>
              <div style={{ fontWeight: 600, fontSize:15.2, color: "#1e40af", marginBottom: 6 }}>How it works:</div>
              <div style={{ fontSize:14.7, color: "#1e40af", lineHeight: 1.7 }}>
            ...
            <div style={{ background: "#f5f3ff", border: "1px solid #c4b5fd", borderRadius: 8, padding: 14, marginTop: 12 }}>
              <div style={{ fontWeight: 600, fontSize:15.2, color: "#5b21b6", marginBottom: 6 }}>Docker Container Governance:</div>
              <div style={{ fontSize:14.7, color: "#5b21b6", lineHeight: 1.7, marginBottom: 8 }}>
              ...
              <div style={{ fontSize:14.7, color: "#5b21b6", lineHeight: 1.7 }}>
                <code style={{ background: "#ede9fe", padding: "2px 6px", borderRadius: 3 }}>sudo cloudfuze-monitor govern</code>
                <div style={{ marginTop: 4, fontSize:14.1, color: "#7c3aed" }}>Interactive &mdash; lists containers, you pick which ones to govern, one at a time. Each container gets the CA cert injected and restarts automatically.</div>
              </div>
            </div>

            {/* Commands reference */}
            <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, padding: 14, marginTop: 12 }}>
              <div style={{ fontWeight: 600, fontSize:15.2, color: "#111", marginBottom: 8 }}>Available Commands:</div>
              <div style={{ fontSize:14.7, color: "#374151", fontFamily: "ui-monospace, monospace", lineHeight: 2.0 }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, padding: 14, marginTop: 12 }}>
              <div style={{ fontWeight: 600, fontSize:15.2, color: "#111", marginBottom: 8 }}>Coverage:</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "4px 16px", fontSize:14.7, lineHeight: 1.8 }}>
              ...
              <div style={{ fontSize:14.1, color: "#6b7280", marginTop: 6 }}>* Requires <code>docker-enable</code> + container restart for prompt/response content.</div>
              <div style={{ fontWeight: 600, fontSize:15.2, color: "#111", marginTop: 12, marginBottom: 8 }}>Not supported:</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "4px 16px", fontSize:14.7, lineHeight: 1.8, color: "#6b7280" }}>
            ...
            <div style={{ background: "#fefce8", border: "1px solid #fde68a", borderRadius: 8, padding: 14, marginTop: 12 }}>
              <div style={{ fontWeight: 600, fontSize:15.2, color: "#854d0e", marginBottom: 6 }}>To uninstall:</div>
              <code style={{ fontSize:14.7, color: "#854d0e" }}>sudo cloudfuze-monitor uninstall</code>
              <div className="aihub_text_muted" style={{ fontSize:14.1, marginTop: 4 }}>Stops the service, removes iptables rules, CA, proxy config, and all files.</div>
            </div>
          </div>
        </div>
      )}
...
function InstallationsView() {
  ...
  return (<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
      ...
      <div className="aihub_card">
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          <div style={{width:44,height:44,borderRadius:10,background:"#eff6ff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24.7}}>💻</div>
          <div>
            <h4 style={{margin:0,fontSize:16.9,fontWeight:700}}>Desktop Agent</h4>
            <div style={{fontSize:14.1,color:"#9ca3af"}}>Windows · macOS · Linux</div>
          </div>
          <div style={{marginLeft:"auto",fontSize:13,color:"#0052e0",background:"#eff6ff",padding:"3px 8px",borderRadius:6,fontWeight:600}}>Step 1</div>
        </div>
        ...
        <details style={{fontSize:14.7,color:"#374151",marginBottom:10}}>
          ...
          <ol style={{lineHeight:2,paddingLeft:18,margin:0}}>
            ...
            <li><strong>Windows:</strong> Extract zip → double-click <code style={{background:"#f1f5f9",padding:"1px 5px",borderRadius:3,fontSize:14.1}}>install.bat</code></li>
            <li><strong>macOS/Linux:</strong> Extract zip → run <code style={{background:"#f1f5f9",padding:"1px 5px",borderRadius:3,fontSize:14.1}}>bash install.sh</code></li>
        ...
        <details style={{fontSize:14.7,color:"#6b7280"}}>
          ...
          <div style={{marginTop:6,lineHeight:1.6}}>
            Silent: <code style={{background:"#f1f5f9",padding:"1px 5px",borderRadius:3,fontSize:14.1}}>CloudFuze-Agent.exe /S</code><br/>
    ...
    <div style={{textAlign:"center",marginTop:14,fontSize:14.1,color:"#9ca3af"}}>
              ...
              return (
                <div key={f.key} style={{
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div className="aihub_text_muted" style={{ fontSize: 13.9, marginTop: 2 }}>
                      {reaches.length
                        ? `Applies on every ${reaches.join(" and ")} in the fleet.`
                        : "Dashboard only — this does not change what is enforced."}
                    </div>
                    {locked ? (
                      <div className="aihub_text_muted" style={{ fontSize: 13.9, marginTop: 4 }}>
...
function FeatureDisabled({ page }) {
  return (
    <div style={{textAlign:"center",padding:"80px 20px"}}>
      ...
      <h3 style={{margin:"0 0 8px",fontSize:19.7,fontWeight:700,color:"#111"}}>Feature not available</h3>
      <p style={{fontSize:15.2,color:"#6b7280",maxWidth:400,margin:"0 auto",lineHeight:1.6}}>
...
function LockedTabContent({ label }) {
  return (
    <div style={{textAlign:"center",padding:"60px 20px"}}>
      ...
      <h3 style={{margin:"0 0 8px",fontSize:18.9,fontWeight:700,color:"#111"}}>Feature Not Available</h3>
