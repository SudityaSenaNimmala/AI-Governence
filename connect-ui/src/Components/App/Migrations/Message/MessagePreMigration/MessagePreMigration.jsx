...
import MessagePreMigrationUser from "./MessagePreMigrationUser";
import MessagePreMigrationChannels from "./MessagePreMigrationChannels";
...
const MessagePreMigration = () => {
  ...
  const removeExistingPreMigration = async () => {
    try {
      setIspageLoading(true);
      let res = await deletePreMigration();
      if (res?.status === "OK") {
        setAdminInfo({
          ...adminInfo,
          processStatus: "NOT_PROCESSED",
        });
        setChannelsPreMigrationInfo([]);
        setUserPreMigrationInfo([]);
        setDmsPreMigrationInfo([]);
        notifyToast("success", "Premigration data deleted successfully");
      } else {
        throw new Error("Failed Deleting Premigration");
      }
    } catch (error) {
          ...
          <div className="cf_userMenu_action_pannel" style={{ gap: "30px" }}>
            ...
            {adminInfo?.processStatus === "NOT_PROCESSED" ? (
              ...
              <>
                ...
                {adminInfo?.processStatus === "PROCESSED" ? (
                  <ActionButton
                    customClass={`changeButtonColorOnHover`}
                    customStyles={{
                      backgroundColor: "#f2f2f2",
                    ...
                    buttonType="button"
                    buttonClickAction={removeExistingPreMigration}
                  >
                    <div
                      style={{
                        display: "flex",
